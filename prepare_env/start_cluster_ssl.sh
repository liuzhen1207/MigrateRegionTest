...
do
	...
	ssh ${u_name}@${c_node} "source /etc/profile;cd ${db_dir};sudo ${db_dir}/sbin/start-confignode.sh -H ${db_dir}/cn_${v_start_time}_heapdump.hprof > /dev/null 2>&1 &"
        sleep 1
#	while true
#	do 
#		sleep 1
...
while read d_node <&4
do
        ssh ${u_name}@${d_node} "source /etc/profile;cd ${db_dir};sudo ${db_dir}/sbin/start-datanode.sh -H ${db_dir}/dn_${v_start_time}_heapdump.hprof > /dev/null 2>&1 &"
        sleep 2 
done
while true
do
sleep 1
v_running=`${client_db_dir}/sbin/start-cli.sh -h ${query_host} -u ${db_sys_admin} ${ssl_str} -e "show cluster;"|grep Running |wc -l`
...
do
   v_running=`${client_db_dir}/sbin/start-cli.sh -h ${query_host} -u ${db_sys_admin} ${ssl_str} -e "show cluster;"|grep Running |wc -l`
   v_readonly=`${client_db_dir}/sbin/start-cli.sh -h ${query_host} -u ${db_sys_admin} ${ssl_str} -e "show cluster;"|grep ReadOnly |wc -l`
