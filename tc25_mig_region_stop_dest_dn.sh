...
function mig_region()
{
   ...
   cat ${cur_dir}/mig.out
   v_check=`grep "${exp_res}" ${cur_dir}/mig.out|wc -l`
...
function pre_and_exec_mig_region()
{
  ...
  stop_dest_dn1 
  mig_region "${v_mig_id}" "${v_mig_from_dn_id2}" "${v_mig_dest_dn_id2}" "IoTDBSQLException: 900: TConsensusGroupId(type:DataRegion, id:${v_mig_id}) has some other region operation procedures in progress"
#  stop_dest_dn2 
