...
function check_data_consistent()
{
   ...
   # all node online,query
    ...
    q3="select count(s_12),count(s_23),count(s_8),count(s_40),count(s_36),count(s_9),max_time(s_17),max_time(s_29),max_time(s_8),max_time(s_49),max_time(s_36),max_time(s_9) from root.test.** align by device;"
    q4="select count(s_12),count(s_23),count(s_8),count(s_40),count(s_36),count(s_9),max_time(s_17),max_time(s_29),max_time(s_8),max_time(s_49),max_time(s_36),max_time(s_9) from root.db.** align by device;"
    ...
    ${cli_dir}/sbin/start-cli.sh -h ${query_ip} -timeout 3600000  -e "${q3}" > ${cur_dir}/q_all_online_q3.out
    ${cli_dir}/sbin/start-cli.sh -h ${query_ip} -timeout 3600000  -e "${q4}" > ${cur_dir}/q_all_online_q4.out
    res_row_num1=`grep root.test ${cur_dir}/q_all_online_test.out|wc -l`
    res_row_num2=`grep root.db ${cur_dir}/q_all_online_db.out|wc -l`
    res_row_num3=`grep root.test ${cur_dir}/q_all_online_q3.out|wc -l`
    res_row_num4=`grep root.db ${cur_dir}/q_all_online_q4.out|wc -l`
   ...
   if [[ ${res_row_num3} = 0 ]] || [[ ${res_row_num4} = 0 ]];then
      let fail_flag++
      return 1
   fi
...
if [[ ${v_ignore} = 0 ]];then
   ...
   ${cli_dir}/sbin/start-cli.sh -h ${q_node} -timeout 3600000  -e "${q3}" > ${cur_dir}/q_stop_ip${v_ip}_q3.out
   ${cli_dir}/sbin/start-cli.sh -h ${q_node} -timeout 3600000  -e "${q4}" > ${cur_dir}/q_stop_ip${v_ip}_q4.out
   ...
   v_diff4=`diff ${cur_dir}/q_all_online_q4.out ${cur_dir}/q_stop_ip${v_ip}_q4.out|grep root|wc -l`
