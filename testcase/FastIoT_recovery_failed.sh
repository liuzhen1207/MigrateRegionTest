...
function test1()
{
  ...
  ${cli_dir}/sbin/start-cli.sh -h ${query_ip} -e "select s1,s2,s3,s4,s5,s6 from root.db.* align by device;"|grep root >${cur_dir}/q_exp.out
  #kill -9 cluster
  exec 3<${nodeinfo_dir}/datanode.txt
  while read node<&3
  do
...
for i in {1..60}
do
   ${cli_dir}/sbin/start-cli.sh -h ${query_ip} -e "select s1,s2,s3,s4,s5,s6 from root.db.* align by device;" |grep root >${cur_dir}/q_act.out
   v_diff=`diff ${cur_dir}/q_exp.out ${cur_dir}/q_act.out|wc -l`
   if [[ ${v_diff} -gt 0 ]];then
      let fail_flag++
   fi 
done
