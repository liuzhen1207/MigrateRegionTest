...
# copy remove-datanode.sh
        ssh ${u_name}@${exec_rm_ip} "mkdir ${db_dir}/tmp"
        ssh ${u_name}@${exec_rm_ip} "cp -rp ${db_dir}/sbin/remove-datanode.sh ${db_dir}/tmp/"
        
        ssh ${u_name}@${exec_rm_ip} "sudo ${db_dir}/tmp/remove-datanode.sh  ${v_rm_datanode_id}">${cur_dir}/rm_cmd_res.out 2>>${cur_dir}/rm_cmd_res.out
        v_rm_fail=`cat ${cur_dir}/rm_cmd_res.out|grep "command not found"|wc -l`
        if [[ ${v_rm_fail} = 0 ]];then
           let fail_flag++
        fi
        for i in {1..10}
