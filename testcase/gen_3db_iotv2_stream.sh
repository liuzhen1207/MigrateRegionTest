...
set_sys_conf ${line} ${db_dir} ".*iot_consensus_v2_mode=.*" "iot_consensus_v2_mode=stream"
...
set_sys_conf ${line} ${db_dir} ".*data_region_group_extension_policy=.*" "data_region_group_extension_policy=CUSTOM"
set_sys_conf ${line} ${db_dir} ".*default_schema_region_group_num_per_database=.*" "default_schema_region_group_num_per_database=1"
set_sys_conf ${line} ${db_dir} ".*default_data_region_group_num_per_database=.*" "default_data_region_group_num_per_database=4"
set_sys_conf ${line} ${db_dir} ".*data_region_consensus_protocol_class=.*" "data_region_consensus_protocol_class=org.apache.iotdb.consensus.iot.IoTConsensusV2"
set_sys_conf ${line} ${db_dir} ".*iot_consensus_v2_mode=.*" "iot_consensus_v2_mode=stream"
  done
...
function run_bm()
{
  ...
  sed -i "s/^HOST=.*/HOST=${iotdb_host}/g" ${bm_dir}/gen_3db/conf1/config.properties
  sed -i "s/^HOST=.*/HOST=${iotdb_host}/g" ${bm_dir}/gen_3db/conf2/config.properties
  sed -i "s/^HOST=.*/HOST=${iotdb_host}/g" ${bm_dir}/gen_3db/conf3/config.properties
  sed -i "s/^HOST=.*/HOST=${iotdb_host}/g" ${bm_dir}/gen_3db/conf4/config.properties
  sed -i "s/^HOST=.*/HOST=${iotdb_host}/g" ${bm_dir}/gen_3db/conf5_tab/config.properties
  ${bm_dir}/benchmark.sh -cf ${bm_dir}/gen_3db/conf1 > ${bm_dir}/${out_file}_1.out
  ${bm_dir}/benchmark.sh -cf ${bm_dir}/gen_3db/conf2 > ${bm_dir}/${out_file}_2.out
  ${bm_dir}/benchmark.sh -cf ${bm_dir}/gen_3db/conf3 > ${bm_dir}/${out_file}_3.out
  ${bm_dir}/benchmark.sh -cf ${bm_dir}/gen_3db/conf4 > ${bm_dir}/${out_file}_4.out
  ${bm_dir}/benchmark.sh -cf ${bm_dir}/gen_3db/conf5_tab > ${bm_dir}/${out_file}_5.out
