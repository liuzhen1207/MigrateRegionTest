...
#  stop_coord_dn 
  # restart unknown
 v_start_time=`date +%s`
  ssh ${u_name}@${v_mig_from_dn_ip1} "source /etc/profile;sudo ${db_dir}/sbin/start-datanode.sh -H ${db_dir}/dn_${v_start_time}_heapdump.hprof > /dev/null 2>&1 &"
  sleep 1
  ...
  sleep 1
   while true
   do
      v_check_status=`${cli_dir}/sbin/start-cli.sh -h ${query_ip} -e "show datanodes"|grep ${v_mig_from_dn_ip1}|grep -i Running|wc -l`
      if [[ ${v_check_status} -gt 0 ]];then
         break
      else
         v_end_time=`date +%s`
