...
# check Removing 
   ...
   do
      ssh ${u_name}@${v_cn_leader_ip} "sudo gunzip ${db_dir}/logs/log-confignode-all*"
              ...
              v_mig_suc_log=`ssh ${u_name}@${v_cn_leader_ip} "grep \"\[MigrateRegion\] success\" ${db_dir}/logs/*confignode*all*|grep \"region ${v_mig_id} has been migrated from DataNode ${v_mig_from_dn_id} to ${v_mig_to_dn_id}\""`
