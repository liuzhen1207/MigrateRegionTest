...
function mig_and_kill()
{
   ...
   v_submit_fail=`grep failed ${cur_dir}/mig.out|wc -l`
   v_submit_suc=`grep success ${cur_dir}/mig.out|wc -l`
   if [[ ${v_submit_fail} = 0 ]] && [[ ${v_submit_suc} = 1 ]];then
      v_mig_flag=success
   fi
   if [[ ${v_submit_fail} = 1 ]] && [[ ${v_submit_suc} = 0 ]];then
      v_mig_flag=fail
   fi
...
v_pid_sec_dn_str=`ssh ${u_name}@${v_mig_sec_dn_ip} "sudo jps|grep -i datanode"`
v_pid_sec_dn=`echo ${v_pid_sec_dn_str}|awk '{print $1}'`
v_pid_remove_dn_str=`ssh ${u_name}@${v_remove_coord_ip} "sudo jps|grep -i datanode"`
