...
while true
do
   ${db_dir}/sbin/start-cli.sh -h ${q_ip} -timeout 36000 -e "show query processlist;" >> q_30_processlist.out &
   sleep 60 
   exec 3<./tmp_ip.txt
   while read line <&3
   do
   ...
   ssh ${u_name}@${line} "sudo jstack -l ${v_pid}" > stack_stop30_${v_ip}_${i}.out
   done
