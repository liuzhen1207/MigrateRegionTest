...
do
   ...
   v_tsfile_num=`ssh ${u_name}@${v_ip} "find ${db_dir}/data/datanode -name *.tsfile|grep -v system|wc -l"`
   if [[ ${v_tsfile_num} = 1 ]];then
      let total_num++
   fi
done
if [[ ${total_num} = 2 ]];then
...
# delete 1 rows
${cli_dir}/sbin/start-cli.sh -h ${query_ip} -sql_dialect tree -e "delete from root.db.d1.col where time=15;"
${cli_dir}/sbin/start-cli.sh -h ${query_ip} -sql_dialect table -e "delete from db.t1 where time=15;"
sleep 1
