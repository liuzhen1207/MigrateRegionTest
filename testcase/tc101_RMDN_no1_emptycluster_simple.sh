...
#done
   ...
   v_variables=`${cli_dir}/sbin/start-cli.sh -h ${query_ip} -e "show variables;"|grep DataRegionConsensusProtocolClass|grep org.apache.iotdb.consensus.simple.SimpleConsensus|wc -l`
   if [[ ${v_variables} -lt 1 ]];then
      let fail_flag++
   fi
}
...
#        v_rm_datanode_id2=6667
        v_rm_res=`ssh ${u_name}@${exec_rm_ip} "sudo ${db_dir}/sbin/start-cli.sh -h ${query_ip} -e \"remove datanode ${v_rm_datanode_id2};\""`
        v_error_msg=`echo ${v_rm_res}|grep "code:1006, message:SimpleConsensus protocol is not supported to remove data node"|wc -l`
        if [[ ${v_error_msg} = 0 ]];then
           let fail_flag++
        fi
...
#        while true
#        do
#            v_running_num=`${cli_dir}/sbin/start-cli.sh -h ${query_ip} -e "show datanodes;"|grep "${exec_rm_ip}|" |wc -l`
#            if [[ ${v_running_num} -gt 0 ]];then
#               sleep 1
#            else
#               break
