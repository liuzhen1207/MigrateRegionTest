...
#        v_rm_res=`ssh ${u_name}@${exec_rm_ip} "sudo ${db_dir}/sbin/remove-datanode.sh ${v_rm_datanode_id}"`
        v_rm_res=`sudo ${cli_dir}/sbin/start-cli.sh -h ${query_ip} -e "remove datanode ${v_rm_datanode_id};"`
        v_rm_fail=`echo "${v_rm_res}"|grep "Failed removing datanode"|wc -l`
	...
	do
        ...
        while true
        do
                v_rm_dn_pid=`ssh ${u_name}@${rm_dn_ip} "sudo jps|grep -i datanode|wc -l"`
	...
	# start removed datanode
		ssh ${u_name}@${rm_dn_ip} "sudo ${db_dir}/sbin/start-datanode.sh -H ${db_dir}/dn_${rm_t1}.hprof > /dev/null 2>&1 &"
                start_t1=`date +%s`
		while true
		do
			...
			if [[ ${v_start_suc} -gt 0 ]];then
			       ...
			       sleep 3 
                        ...
                        start_t2=`date +%s`
                        start_elp=$((start_t2-start_t1))
                        if [[ ${start_elp} -gt 120 ]];then
                           let fail_flag++
                           break
                        fi
		done
...
remove_datanode ${last_dn_ip} ${query_cn_ip}
test_end_sec=`date +%s`
