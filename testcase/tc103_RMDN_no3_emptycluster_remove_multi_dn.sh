...
function remove_datanode()
{
	...
	exec_rm_ip=$2
        ...
        v_rm_res=`ssh ${u_name}@${exec_rm_ip} "sudo ${db_dir}/sbin/start-cli.sh -h ${query_ip} -e \"remove datanode ${v_rm_datanode_id},${v_rm_datanode_id2};\""`
        v_err_msg=`echo ${v_rm_res}|grep "Error occurred while parsing SQL"|wc -l`
        if [[ ${v_err_msg} = 0 ]];then
           let fail_flag++
        fi
        # remove same datanode id
        v_rm_res=`ssh ${u_name}@${exec_rm_ip} "sudo ${db_dir}/sbin/start-cli.sh -h ${query_ip} -e \"remove datanode ${v_rm_datanode_id} ${v_rm_datanode_id};\""`
        v_err_msg=`echo ${v_rm_res}|grep "Error occurred while parsing SQL"|wc -l`
        if [[ ${v_err_msg} = 0 ]];then
           let fail_flag++
        fi

	v_dn_status=`${cli_dir}/sbin/start-cli.sh -h ${query_ip} -e "show datanodes;"|grep -i running|wc -l`
	if [[ ${v_dn_status} != ${dn_num} ]];then
           let fail_flag++
	fi
