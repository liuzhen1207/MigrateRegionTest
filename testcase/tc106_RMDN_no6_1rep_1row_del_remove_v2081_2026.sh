...
function ins_data()
{
   ...
   for i in {1..5}
   do
      run_sql_expect_success "create database root.db${i};" "${cur_dir}/tc106_create_db${i}.out" || return 1
      run_sql_expect_success "create aligned timeseries root.db${i}.t1(text TEXT,grade int32,male boolean,likething string,money double,rate float,age int64,movie blob,birthday date,run timestamp);" "${cur_dir}/tc106_create_ts${i}.out" || return 1
      run_sql_expect_success "insert into root.db${i}.t1(time,text,grade,male,likething,money,rate,age,movie,birthday,run) values(10000,'expensive',1,false,'calculate',1.23,2.1,21,X'1234','2024-12-01',2021-12-01 13:14:15);" "${cur_dir}/tc106_insert_db${i}.out" || return 1
      run_sql_expect_success "flush;" "${cur_dir}/tc106_flush_db${i}.out" || return 1
   done
   ...
   if [[ $(grep -o "expensive" "${cur_dir}/q_before_delete.out" | wc -l) -ne 5 ]];then
      echo "Expected 5 values before deleting timeseries."
   ...
   for i in {1..5}
   do
      run_sql_expect_success "delete timeseries root.db${i}.t1.*;" "${cur_dir}/tc106_delete_db${i}.out" || return 1
   done
   ...
   if grep -q "expensive" "${cur_dir}/q_act.out";then
      echo "Deleted row is still queryable."
      let fail_flag++
      return 1
   fi
   return 0
...
function remove_datanode()
{
	...
	local exec_rm_ip=$2
        ...
        do
           ...
           ${cli_dir}/sbin/start-cli.sh -h ${query_ip} -e "show datanodes;" >"${cur_dir}/tc106_show_dn_${ip_suffix}.out" 2>&1
           v_dn_num=`grep "${rm_dn_ip}|" "${cur_dir}/tc106_show_dn_${ip_suffix}.out"|wc -l`
           ${cli_dir}/sbin/start-cli.sh -h ${query_ip} -sql_dialect tree -e "show regions;" >"${cur_dir}/tc106_regions_tree_${ip_suffix}.out" 2>&1
           ${cli_dir}/sbin/start-cli.sh -h ${query_ip} -sql_dialect table -e "show regions;" >"${cur_dir}/tc106_regions_table_${ip_suffix}.out" 2>&1
           active_num=`grep -E "Adding|Removing" "${cur_dir}/tc106_regions_tree_${ip_suffix}.out" "${cur_dir}/tc106_regions_table_${ip_suffix}.out"|wc -l`
...
if [[ ${fail_flag} = 0 ]];then
   ...
   # The deleted dataset must remain empty after both removes.
