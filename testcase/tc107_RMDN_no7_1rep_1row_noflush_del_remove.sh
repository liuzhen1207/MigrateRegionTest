...
function check_data_consistent()
{
   ...
   # all node online,query
    q1="select device_id,count(movie) from db1.t1 group by device_id order by device_id;"
    q2="select device_id,count(movie) from db2.t1 group by device_id order by device_id;"
    q3="select device_id,count(movie) from db3.t1 group by device_id order by device_id;"
    q4="select device_id,count(movie) from db4.t1 group by device_id order by device_id;"
    q5="select device_id,count(movie) from db5.t1 group by device_id order by device_id;"
    ${cli_dir}/sbin/start-cli.sh -h ${query_ip} -sql_dialect table -timeout 3600  -e "${q1}" > ${cur_dir}/q_all_online_q1.out
    ${cli_dir}/sbin/start-cli.sh -h ${query_ip} -sql_dialect table -timeout 3600  -e "${q2}" > ${cur_dir}/q_all_online_q2.out
    ${cli_dir}/sbin/start-cli.sh -h ${query_ip} -sql_dialect table -timeout 3600  -e "${q3}" > ${cur_dir}/q_all_online_q3.out
    ${cli_dir}/sbin/start-cli.sh -h ${query_ip} -sql_dialect table -timeout 3600  -e "${q4}" > ${cur_dir}/q_all_online_q4.out
    ${cli_dir}/sbin/start-cli.sh -h ${query_ip} -sql_dialect table -timeout 3600  -e "${q5}" > ${cur_dir}/q_all_online_q5.out
    res_row_num1=`grep 0001 ${cur_dir}/q_all_online_q1.out|wc -l`
    res_row_num2=`grep 0001 ${cur_dir}/q_all_online_q2.out|wc -l`
    res_row_num3=`grep 0001 ${cur_dir}/q_all_online_q3.out|wc -l`
    res_row_num4=`grep 0001 ${cur_dir}/q_all_online_q4.out|wc -l`
    res_row_num5=`grep 0001 ${cur_dir}/q_all_online_q5.out|wc -l`
   if [[ ${res_row_num1} -gt 0 ]];then
      let fail_flag++
      return 1
   fi
   if [[ ${res_row_num2} -gt 0 ]];then
      let fail_flag++
      return 1
   fi
   if [[ ${res_row_num3} -gt 0 ]];then
      let fail_flag++
      return 1
   fi
   if [[ ${res_row_num4} -gt 0 ]];then
      let fail_flag++
      return 1
   fi
   if [[ ${res_row_num5} -gt 0 ]];then
      let fail_flag++
      return 1
   fi
   for i in {1..5}
   do
      v_count_dev=`${cli_dir}/sbin/start-cli.sh -h ${query_ip} -sql_dialect table -e "use db${i};count devices from t1;"|grep " 0|"|wc -l`
      if [[ ${v_count_dev} != 1 ]];then
         let fail_flag++
      fi
   done
...
#   ${cli_dir}/sbin/start-cli.sh -h ${q_node} -sql_dialect table -timeout 3600  -e "${q1}" > ${cur_dir}/q_stop_ip${v_ip}_q1.out
#   ${cli_dir}/sbin/start-cli.sh -h ${q_node} -sql_dialect table -timeout 3600  -e "${q2}" > ${cur_dir}/q_stop_ip${v_ip}_q2.out
#   ${cli_dir}/sbin/start-cli.sh -h ${q_node} -sql_dialect table -timeout 3600  -e "${q3}" > ${cur_dir}/q_stop_ip${v_ip}_q3.out
#   ${cli_dir}/sbin/start-cli.sh -h ${q_node} -sql_dialect table -timeout 3600  -e "${q4}" > ${cur_dir}/q_stop_ip${v_ip}_q4.out
#   ${cli_dir}/sbin/start-cli.sh -h ${q_node} -sql_dialect table -timeout 3600  -e "${q5}" > ${cur_dir}/q_stop_ip${v_ip}_q5.out
#   v_diff1=`diff ${cur_dir}/q_all_online_q1.out ${cur_dir}/q_stop_ip${v_ip}_q1.out|grep 0001|wc -l`
#   v_diff2=`diff ${cur_dir}/q_all_online_q2.out ${cur_dir}/q_stop_ip${v_ip}_q2.out|grep 0001|wc -l`
#   v_diff3=`diff ${cur_dir}/q_all_online_q3.out ${cur_dir}/q_stop_ip${v_ip}_q3.out|grep 0001|wc -l`
#   v_diff4=`diff ${cur_dir}/q_all_online_q4.out ${cur_dir}/q_stop_ip${v_ip}_q4.out|grep 0001|wc -l`
#   v_diff5=`diff ${cur_dir}/q_all_online_q5.out ${cur_dir}/q_stop_ip${v_ip}_q5.out|grep 0001|wc -l`
...
for i in {1..5}
do
   ${cli_dir}/sbin/start-cli.sh -h ${query_ip} -sql_dialect table -e "create database db${i};use db${i};create table t1(region string tag, device_id string tag, color string attribute, text TEXT field,grade int32 field,male boolean field,likething string field,money double field,rate float field,age int64 field,movie blob field,birthday date field,run timestamp field );insert into t1(region,device_id,color,text,grade,male,likething,money,rate,age,movie,birthday,run) values('cpu','0001','blue','expensive',1,false,'calculate',1.23,2.1,21,x'1234','2024-12-01','2021-12-01 13:14:15');"
   ${cli_dir}/sbin/start-cli.sh -h ${query_ip} -sql_dialect table -e "use db${i};delete devices from t1 where device_id='0001' ;"
done   
}
