...
function check_data_consistent()
{
   ...
   for i in {1..5}
   do
      v_count_dev=`${cli_dir}/sbin/start-cli.sh -h ${query_ip} -sql_dialect table -e "use db${i};count devices from t1;"|grep " 1|"|wc -l`
...
do
   ...
   ${cli_dir}/sbin/start-cli.sh -h ${query_ip} -sql_dialect table -e "use db${i};delete from t1 ;"
done   
}
