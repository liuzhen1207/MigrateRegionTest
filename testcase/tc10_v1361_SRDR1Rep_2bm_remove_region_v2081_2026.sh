...
sed -i "s/^PASSWORD=.*/PASSWORD=${bm_root_pw}/g" ${bm_dir}/lt_10type_user_no_ssl/conf*/config.properties
   ...
   nohup sh -x ${bm_dir}/benchmark.sh -cf ${bm_dir}/lt_10type_user_no_ssl/conf1 >${bm_dir}/${v_t}_tc10_bm1.out &
   nohup sh -x ${bm_dir}/benchmark.sh -cf ${bm_dir}/lt_10type_user_no_ssl/conf2 >${bm_dir}/${v_t}_tc10_bm2.out &
   sleep 30
...
# extend region
   ...
   check_res2 "only has 1 replica, it cannot be removed" "has some other region operation procedures in progress" 1 "${SCRIPT_NAME}"
   fi
   ...
   wait_bm_finish 36000 "${bm_dir}/${v_t}_tc10_bm1.out" "${bm_dir}/${v_t}_tc10_bm2.out"
