...
remove_datanode_under_load() {
  ...
  # tc110 removes a live DataNode while benchmark writes are still running.
  # Unlike tc109, do not stop the target and do not wait for Unknown here.
...
collect_sync_lag_jstack() {
  ...
  mkdir -p "${output_dir}"
  cat "${nodeinfo_dir}/confignode.txt" "${nodeinfo_dir}/datanode.txt" | awk 'NF' | sort -u > "${output_dir}/nodes.txt"
  cp -f "${cur_dir}/sync_lag_response.json" "${output_dir}/sync_lag_response.json" 2>/dev/null || true

  while read -r node_ip
  do
    [[ -z "${node_ip}" ]] && continue
    timeout 120 ssh "${u_name}@${node_ip}" 'source /etc/profile; hostname; for pid in $(jps -l | awk "/ConfigNode|DataNode/ {print \$1}"); do echo "===== PID ${pid} ====="; jstack -l "${pid}"; done' > "${output_dir}/${node_ip}.jstack" 2>&1 &
  done < "${output_dir}/nodes.txt"
  wait
  log "syncLag timeout jstack saved in ${output_dir}"
}
