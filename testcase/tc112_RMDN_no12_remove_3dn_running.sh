...
remove_datanode_under_load() {
  ...
  rm_target_ip=${1:-$(tail -1 "${nodeinfo_dir}/datanode.txt")}
  ...
  if ! wait_datanode_status "${query_ip}" "${rm_target_ip}" "Running" 300; then
    log "datanode ${rm_target_ip} is not Running before remove"
    let fail_flag++
    return 1
  fi
...
remove_datanode_expect_fail() {
  ...
  if [[ -z "${rm_dn_id}" ]]; then
    log "failed to locate expected-failure remove target id for ${target_ip}"
    let fail_flag++
    return 1
  fi
  if ! wait_datanode_status "${query_ip}" "${target_ip}" "Running" 300; then
    log "third remove target ${target_ip} is not Running"
    let fail_flag++
    return 1
  fi

  run_cli_sql "${query_ip}" tree "remove datanode ${rm_dn_id};" "${cur_dir}/remove_datanode_expected_fail.out" 3600
  if grep -qi 'successfully' "${cur_dir}/remove_datanode_expected_fail.out"; then
    log "third remove unexpectedly succeeded for ${target_ip}"
    let fail_flag++
    return 1
  fi

  # The rejected target must remain Running and must not enter Removing.
  for sample in 1 2 3
  do
    sleep 5
    run_cli_sql "${query_ip}" tree "show datanodes;" "${cur_dir}/show_datanodes.out" 3600
    if ! grep "${target_ip}|" "${cur_dir}/show_datanodes.out" | grep -q 'Running'; then
      log "third remove target ${target_ip} did not remain Running"
      let fail_flag++
      return 1
    fi
    if grep "${target_ip}|" "${cur_dir}/show_datanodes.out" | grep -q 'Removing'; then
      log "third remove target ${target_ip} unexpectedly entered Removing"
      let fail_flag++
      return 1
    fi
  done
  log "third remove was rejected and ${target_ip} remained Running as expected"
  return 0
}
...
testcase() {
  ...
  remove_datanode_under_load "${last_dn_ip}"
  remove_datanode_under_load "${last_dn_ip2}"
  remove_datanode_expect_fail "${last_dn_ip3}"
