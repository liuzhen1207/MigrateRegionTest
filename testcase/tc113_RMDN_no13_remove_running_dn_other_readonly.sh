...
set_other_datanodes_readonly() {
  local target_ip=$1
  local dn_ip

  while read -r dn_ip
  do
    [[ -z "${dn_ip}" || "${dn_ip}" = "${target_ip}" ]] && continue
    run_cli_sql "${dn_ip}" tree "set system to readonly on local;" "${cur_dir}/set_readonly_${dn_ip##*.}.out" 300
    if ! wait_datanode_status "${target_ip}" "${dn_ip}" "ReadOnly" 300; then
      log "datanode ${dn_ip} did not become ReadOnly"
      let fail_flag++
      return 1
    fi
...
restore_other_datanodes_running() {
  local target_ip=$1
  local dn_ip
  ...
  do
    [[ -z "${dn_ip}" || "${dn_ip}" = "${target_ip}" ]] && continue
    run_cli_sql "${dn_ip}" tree "set system to running on local;" "${cur_dir}/set_running_${dn_ip##*.}.out" 300
    if ! wait_datanode_status "${target_ip}" "${dn_ip}" "Running" 300; then
      log "datanode ${dn_ip} did not return to Running"
      let fail_flag++
      restore_failed=1
    fi
  done < "${nodeinfo_dir}/datanode.txt"
...
remove_running_datanode_expect_fail() {
  local target_ip=$1
  local rm_dn_id
  local sample
  ...
  if grep -qi 'successfully' "${cur_dir}/remove_datanode_expected_fail.out"; then
    log "remove unexpectedly succeeded for ${target_ip} while all other DataNodes were ReadOnly"
    let fail_flag++
    return 1
  fi
  ...
  log "remove was rejected and ${target_ip} remained Running as expected"
  return 0
}
...
testcase() {
  ...
  set_other_datanodes_readonly "${target_ip}"
  if ! remove_running_datanode_expect_fail "${target_ip}"; then
    restore_other_datanodes_running "${target_ip}"
    return 1
  fi
  if ! restore_other_datanodes_running "${target_ip}"; then
    return 1
  fi
