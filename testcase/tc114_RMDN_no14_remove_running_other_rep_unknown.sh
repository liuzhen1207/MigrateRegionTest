...
wait_datanode_process_exit() {
  ...
  while true
  do
    jps_num=$(ssh "${u_name}@${dn_ip}" "source /etc/profile; sudo jps -l | grep -c 'DataNode' || true")
    if [[ "${jps_num}" =~ ^[0-9]+$ ]] && [[ ${jps_num} -eq 0 ]]; then
      return 0
    fi
    if (( $(date +%s) - start_time > timeout_sec )); then
      return 1
...
remove_running_with_replica_unknown_expect_fail() {
  local target_ip=$1
  local rm_dn_id
  local region_id
  local peer_ip
  ...
  log "stop replica peer ${peer_ip} of region ${region_id} before removing ${target_ip}"
  ssh "${u_name}@${peer_ip}" "source /etc/profile; cd ${db_dir}; sudo ./sbin/stop-datanode.sh"
  if ! wait_datanode_status "${target_ip}" "${peer_ip}" "Unknown" 300; then
    log "replica peer ${peer_ip} did not become Unknown"
    let fail_flag++
    return 1
  fi
  if ! wait_datanode_process_exit "${peer_ip}" 120; then
    log "DataNode process did not exit on replica peer ${peer_ip} within 120s"
    let fail_flag++
    return 1
  fi
  ...
  if grep -qi 'successfully' "${cur_dir}/remove_datanode_expected_fail.out"; then
    log "remove unexpectedly succeeded for ${target_ip} with replica peer ${peer_ip} Unknown"
    let fail_flag++
    return 1
  fi
...
testcase() {
  ...
  remove_running_with_replica_unknown_expect_fail "${target_ip}"
