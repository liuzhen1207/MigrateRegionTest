...
#        v_rm_datanode_id2=6667
        v_rm_res=`ssh ${u_name}@${exec_rm_ip} "sudo ${db_dir}/sbin/remove-datanode.sh ${v_rm_datanode_id2} ${v_rm_datanode_id}"`
        v_rm_fail=`echo "${v_rm_res}"|grep "Invalid node-id"|wc -l`
        if [[ ${v_rm_fail} -gt 0 ]];then
           let fail_flag++
        fi
#        for i in {1..10}
#        do
