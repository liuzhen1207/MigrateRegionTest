...
# check remove 
        ...
        while true 
        do
            v_dn_num1=`${cli_dir}/sbin/start-cli.sh -h ${query_ip} -e "show datanodes;"|grep "${rm_dn_ip}|" |wc -l`
            v_dn_num2=`${cli_dir}/sbin/start-cli.sh -h ${exec_rm_ip} -e "show datanodes;"|grep "${rm_dn_ip}|" |wc -l`
            v_dn_num3=`ssh ${u_name}@${stop_other_dn_ip} "sudo jps|grep -i datanode|wc -l"`
            v_dn_num=$((v_dn_num1+v_dn_num2+v_dn_num3))
