...
remove_running_with_replica_readonly_expect_fail() {
  local target_ip=$1
  local rm_dn_id
  local region_id
  local peer_ip
  ...
  run_cli_sql "${target_ip}" tree "show datanodes;" "${cur_dir}/show_datanodes.out" 3600
  ...
  run_cli_sql "${target_ip}" table "show data regions;" "${cur_dir}/show_data_regions_table.out" 3600
  region_id=$(grep "${target_ip}|" "${cur_dir}/show_data_regions_table.out" | head -1 | awk -F '|' '{gsub(" ","",$2);print $2}')
  if [[ -n "${region_id}" ]]; then
    peer_ip=$(grep -E "${region_id}\|[[:space:]]*DataRegion" "${cur_dir}/show_data_regions_table.out" | grep -v "${target_ip}" | head -1 | awk -F '|' '{gsub(" ","",$9);print $9}')
  else
    run_cli_sql "${target_ip}" tree "show data regions;" "${cur_dir}/show_data_regions_tree.out" 3600
    region_id=$(grep -v system "${cur_dir}/show_data_regions_tree.out" | grep "${target_ip}|" | head -1 | awk -F '|' '{gsub(" ","",$2);print $2}')
    peer_ip=$(grep -E "${region_id}\|[[:space:]]*DataRegion" "${cur_dir}/show_data_regions_tree.out" | grep -v "${target_ip}" | head -1 | awk -F '|' '{gsub(" ","",$9);print $9}')
  fi

  if [[ -z "${region_id}" || -z "${peer_ip}" ]]; then
    log "failed to locate another replica for a DataRegion on ${target_ip}"
    let fail_flag++
    return 1
  fi
  log "set replica peer ${peer_ip} of region ${region_id} ReadOnly before removing ${target_ip}"
  run_cli_sql "${peer_ip}" tree "set system to readonly on local;" "${cur_dir}/set_replica_readonly.out" 300
  if ! wait_datanode_status "${target_ip}" "${peer_ip}" "ReadOnly" 300; then
    log "replica peer ${peer_ip} did not become ReadOnly"
    let fail_flag++
    return 1
  fi
  if ! wait_datanode_status "${target_ip}" "${target_ip}" "Running" 300; then
    log "remove target ${target_ip} is not Running"
    let fail_flag++
    return 1
  fi

  run_cli_sql "${target_ip}" tree "remove datanode ${rm_dn_id};" "${cur_dir}/remove_datanode_expected_fail.out" 3600
  if grep -qi 'successfully' "${cur_dir}/remove_datanode_expected_fail.out"; then
    log "remove unexpectedly succeeded for ${target_ip} with replica peer ${peer_ip} ReadOnly"
    let fail_flag++
    return 1
  fi
  if ! grep -q 'Submit RemoveDataNodesProcedure failed' "${cur_dir}/remove_datanode_expected_fail.out"; then
    log "remove failed without the expected Submit RemoveDataNodesProcedure failure"
    cat "${cur_dir}/remove_datanode_expected_fail.out"
    let fail_flag++
    return 1
  fi
  ...
  do
    sleep 5
    run_cli_sql "${target_ip}" tree "show datanodes;" "${cur_dir}/show_datanodes.out" 3600
    if ! grep "${target_ip}|" "${cur_dir}/show_datanodes.out" | grep -q 'Running'; then
      log "rejected remove target ${target_ip} did not remain Running"
      let fail_flag++
      return 1
    fi
    if grep "${target_ip}|" "${cur_dir}/show_datanodes.out" | grep -q 'Removing'; then
      log "rejected remove target ${target_ip} unexpectedly entered Removing"
      let fail_flag++
      return 1
    fi
    if ! grep "${peer_ip}|" "${cur_dir}/show_datanodes.out" | grep -q 'ReadOnly'; then
      log "replica peer ${peer_ip} did not remain ReadOnly"
      let fail_flag++
      return 1
    fi
  done

  # The ReadOnly state is only needed to verify remove rejection. Restore the
  # replica before the remaining benchmarks and sync-lag checks continue.
  log "restore replica peer ${peer_ip} to Running"
  run_cli_sql "${peer_ip}" tree "set system to running on local;" "${cur_dir}/set_replica_running.out" 300
  if ! wait_datanode_status "${target_ip}" "${peer_ip}" "Running" 300; then
    log "replica peer ${peer_ip} did not return to Running"
    let fail_flag++
    return 1
  fi
  log "remove was rejected; ${target_ip} remained Running and replica peer ${peer_ip} was restored to Running"
  return 0
}
...
wait_benchmarks_finish() {
  ...
  do
    ...
    do
      ...
      # Fail fast when a benchmark exits before writing its completion markers.
      if [[ ${bm_check_rc} -ne 0 && ! -f "${bm_pid_file}" ]]; then
        log "benchmark PID file missing: ${bm_pid_file}"
        let fail_flag++
        return 1
      fi
      local bm_pid
      bm_pid=$(cat "${bm_pid_file}" 2>/dev/null)
      if [[ ${bm_check_rc} -ne 0 && ! "${bm_pid}" =~ ^[0-9]+$ ]]; then
        log "invalid benchmark PID for ${workload}: ${bm_pid}"
        let fail_flag++
        return 1
      fi
      if [[ ${bm_check_rc} -ne 0 ]] && ! kill -0 "${bm_pid}" 2>/dev/null; then
        log "benchmark exited before completion: ${workload} (pid=${bm_pid})"
        tail -n 40 "${bm_file}" 2>/dev/null || true
        let fail_flag++
        return 1
      fi
    done
...
testcase() {
  ...
  target_ip=$(tail -1 "${nodeinfo_dir}/datanode.txt")
  remove_running_with_replica_readonly_expect_fail "${target_ip}"
