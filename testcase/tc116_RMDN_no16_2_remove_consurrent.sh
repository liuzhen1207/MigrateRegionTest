...
# check remove
		...
		while true 
		do
		    local v_dn_num1=`${cli_dir}/sbin/start-cli.sh -h ${query_ip} -e "show datanodes;"|grep "${rm_dn_ip}|" |wc -l`
		    local v_dn_num2=`${cli_dir}/sbin/start-cli.sh -h ${exec_rm_ip} -e "show datanodes;"|grep "${rm_dn_ip}|" |wc -l`
		    local v_dn_num3=`ssh ${u_name}@${rm_dn_ip} "sudo jps|grep -i datanode|wc -l"`
		    local v_dn_num=$((v_dn_num1+v_dn_num2+v_dn_num3))
		    if [[ ${v_dn_num} -gt 0 ]];then
		       sleep 10 
		    else
		       echo "${rm_dn_ip}," >> ${cur_dir}/ignore_dn_list.txt
                       # check data dir
                       v_tsfile_num=`ssh ${u_name}@${rm_dn_ip} "sudo find ${db_dir}/data -name *.tsfile|wc -l"`
                       if [[ ${v_tsfile_num} -gt 0 ]];then
                          let fail_flag++
                       fi
		       break 
		    fi
...
sleep 1
remove_datanode ${last_dn_ip2} ${last_dn_ip2} "fail"
wait
