...
function set_dn_bad()
{
        ...
        while read line<&3
        do
            if [[ "${line}" != "${rm_dn_ip}" ]];then
               ${cli_dir}/sbin/start-cli.sh -h ${line} -e "set system to readonly on local;"
            fi
        done
...
function remove_datanode()
{
	...
	exec_rm_ip=$2
        rm_flag="success"
        v_rm_datanode_id=`${cli_dir}/sbin/start-cli.sh -h ${query_ip} -e "show datanodes;"|grep "${rm_dn_ip}|"|awk -F '|' '{gsub(" ","");print $2}'`
        find_a_dr_on_remove_dn=`${cli_dir}/sbin/start-cli.sh -h ${query_ip} -timeout 3600 -sql_dialect table -e 'show data regions;'|grep "${rm_dn_ip}|"|head -1|awk -F '|' '{gsub(" ","");print $2}'`
        if [[ ${find_a_dr_on_remove_dn} != "" ]];then
           stop_other_dn_ip=`${cli_dir}/sbin/start-cli.sh -h ${query_ip} -timeout 3600 -sql_dialect table -e 'show data regions;'|grep "${find_a_dr_on_remove_dn}|\s*DataRegion"|grep -v ${rm_dn_ip}|head -1|awk -F '|' '{gsub(" ","");print $9}'`
        else
           find_a_dr_on_remove_dn=`${cli_dir}/sbin/start-cli.sh -h ${query_ip} -timeout 3600 -sql_dialect tree -e 'show data regions;'|grep -v system|grep "${rm_dn_ip}|"|head -1|awk -F '|' '{gsub(" ","");print $2}'` 
           stop_other_dn_ip=`${cli_dir}/sbin/start-cli.sh -h ${query_ip} -timeout 3600 -sql_dialect tree -e 'show data regions;'|grep "${find_a_dr_on_remove_dn}|\s*DataRegion"|grep -v ${rm_dn_ip}|head -1|awk -F '|' '{gsub(" ","");print $9}'`
        fi
        ${cli_dir}/sbin/start-cli.sh -h ${rm_dn_ip} -e "remove datanode  ${v_rm_datanode_id};">${cur_dir}/rm_cmd_res.out 2>>${cur_dir}/rm_cmd_res.out
        v_rm_fail=`cat ${cur_dir}/rm_cmd_res.out|grep "success"|wc -l`
        if [[ ${v_rm_fail} = 0 ]];then
           let fail_flag++
           return 1
        fi
        v_dn_num=`${cli_dir}/sbin/start-cli.sh -h ${query_ip} -e "show datanodes;"|grep "${rm_dn_ip}|"|grep "Removing" |wc -l`
        sleep 5
         # stop other dn
         if [[ ${stop_other_dn_ip} != "" ]];then
                # stop stop_other_dn_ip
                ${cli_dir}/sbin/start-cli.sh -h ${stop_other_dn_ip} -timeout 3600 -sql_dialect tree -e 'set system to readonly on local;'
                while true
                do
                   v_unknown=`${cli_dir}/sbin/start-cli.sh -h ${stop_other_dn_ip} -timeout 3600 -sql_dialect tree -e 'show datanodes;'|grep "${stop_other_dn_ip}|"|grep -i readonly|wc -l`
                   if [[ ${v_unknown} = 1 ]];then
                      break
                   else
                      sleep  3
...
# check remove 
        ...
        do
            v_dn_num=`${cli_dir}/sbin/start-cli.sh -h ${query_ip} -e "show datanodes;"|grep "${rm_dn_ip}|"|grep "Removing" |wc -l`
            v_dr_adding=`${cli_dir}/sbin/start-cli.sh -h ${query_ip} -sql_dialect tree -e "show data regions;"|grep "Adding" |wc -l`
            v_dr_removing=`${cli_dir}/sbin/start-cli.sh -h ${query_ip} -sql_dialect tree -e "show data regions;"|grep "Removing" |wc -l`
            v_dr_adding2=`${cli_dir}/sbin/start-cli.sh -h ${query_ip} -sql_dialect table -e "show data regions;"|grep "Adding" |wc -l`
            v_dr_removing2=`${cli_dir}/sbin/start-cli.sh -h ${query_ip} -sql_dialect table -e "show data regions;"|grep "Removing" |wc -l`
            v_rm_status=$((v_dn_num+v_dr_adding+v_dr_adding2+v_dr_removing+v_dr_removing2))
          
            if [[ ${v_rm_status} -gt 0 ]];then
               sleep 10 
            elif [[ ${v_rm_status} = 0 ]];then
               v_dn_num=`${cli_dir}/sbin/start-cli.sh -h ${query_ip} -e "show datanodes;"|grep "${rm_dn_ip}|"|grep "Removing" |wc -l`
               if [[ ${v_dn_num} = 0 ]];then
...
function remove_datanode_exp_fail()
{
        rm_dn_ip=$1
        exec_rm_ip=$2
        rm_flag="success"
...
# check remove
        ...
        do
            ...
            if [[ ${v_dn_num} -gt 0 ]];then
               sleep 2 
               v_dn_status=`${cli_dir}/sbin/start-cli.sh -h ${query_ip} -e "show datanodes;"|grep "${rm_dn_ip}|"|grep Running |wc -l`
               v_dn_status2=`${cli_dir}/sbin/start-cli.sh -h ${query_ip} -e "show datanodes;"|grep "${rm_dn_ip}|"|grep Removing |wc -l`
               if [[ ${v_dn_status} = 1 ]];then
                  break
               fi
               if [[ ${v_dn_status2} = 1 ]];then
                  let fail_flag++
                  break
               fi
            else
            ...
            let v_loop++
            if [[ ${v_loop} -gt 10 ]];then
               echo "cluster status is not right."
               let fail_flag++
               break
            fi
        done
