...
#        ssh ${u_name}@${rm_dn_ip} "sudo ${db_dir}/sbin/stop-datanode.sh"
        ...
        while true
        do
           v_unknown=`${cli_dir}/sbin/start-cli.sh -h ${query_ip} -e "show datanodes;"|grep "${rm_dn_ip}|"|grep -i only|wc -l`
...
do
	   ...
	   let loop++
	   if [[ ${loop} -gt 120 ]];then
	      echo "ERROR : remove readonly ${rm_dn_ip} fail."
	      let fail_flag++
	      return 1
	   fi
done
