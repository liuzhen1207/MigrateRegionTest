...
function remove_datanode()
{
	...
	local exec_rm_ip=$2
        ...
        local v_rm_datanode_id=`${cli_dir}/sbin/start-cli.sh -h ${query_ip} -e "show datanodes;"|grep "${rm_dn_ip}|"|awk -F '|' '{gsub(" ","");print $2}'`
        ${cli_dir}/sbin/start-cli.sh -h ${rm_dn_ip} -e "remove datanode  ${v_rm_datanode_id};">${cur_dir}/rm_cmd_res_${v_rm_datanode_id}.out 2>>${cur_dir}/rm_cmd_res_${v_rm_datanode_id}.out
        local v_rm_res=`cat ${cur_dir}/rm_cmd_res_${v_rm_datanode_id}.out|grep -i ${rm_flag}|wc -l`
        if [[ ${v_rm_res} = 0 ]];then
           let fail_flag++
           return 1
        fi
...
# check remove
        if [[ ${rm_flag} = success ]];then
                ...
                while true
                do
                        find_a_dr_is_adding=`${cli_dir}/sbin/start-cli.sh -h ${query_ip} -timeout 3600 -sql_dialect table -e 'show data regions;'|grep "Adding"|head -1|awk -F '|' '{gsub(" ","");print $2}'`
                        find_a_dr_is_adding_ip=`${cli_dir}/sbin/start-cli.sh -h ${query_ip} -timeout 3600 -sql_dialect table -e 'show data regions;'|grep "Adding"|head -1|awk -F '|' '{gsub(" ","");print $9}'`
			if [[ ${find_a_dr_is_adding} != "" ]] && [[ ${find_a_dr_is_adding_ip} != "" ]];then
			   ${cli_dir}/sbin/start-cli.sh -h ${find_a_dr_is_adding_ip} -timeout 3600 -sql_dialect table -e 'set system to readonly on local;'
                           let rm_flag=fail
                           break
			else
                           sleep 1
                        ...
                        find_a_dr_is_adding=`${cli_dir}/sbin/start-cli.sh -h ${query_ip} -timeout 3600 -sql_dialect tree -e 'show data regions;'|grep "Adding"|head -1|awk -F '|' '{gsub(" ","");print $2}'`
                        find_a_dr_is_adding_ip=`${cli_dir}/sbin/start-cli.sh -h ${query_ip} -timeout 3600 -sql_dialect tree -e 'show data regions;'|grep "Adding"|head -1|awk -F '|' '{gsub(" ","");print $9}'`
                        if [[ ${find_a_dr_is_adding} != "" ]] && [[ ${find_a_dr_is_adding_ip} != "" ]];then
                           ${cli_dir}/sbin/start-cli.sh -h ${find_a_dr_is_adding_ip} -timeout 3600 -sql_dialect tree -e 'set system to readonly on local;'
                           let rm_flag=fail
                           break
                        else
                           sleep 1
		...
		while true 
		do
		    local v_rm_dn_running_num=`${cli_dir}/sbin/start-cli.sh -h ${query_ip} -e "show datanodes;"|grep "${rm_dn_ip}|"|grep Running |wc -l`
		    local v_rm_dn_removing_num=`${cli_dir}/sbin/start-cli.sh -h ${query_ip} -e "show datanodes;"|grep "${rm_dn_ip}|"|grep Removing |wc -l`
		    local v_rm_dn_num=`${cli_dir}/sbin/start-cli.sh -h ${query_ip} -e "show datanodes;"|grep "${rm_dn_ip}|"|wc -l`
		    if [[ ${v_rm_dn_running_num} = 1 ]];then
		       break 
		    fi
                    if [[ ${v_rm_dn_removing_num} = 1 ]];then
                       sleep 1
                       let v_loop++
                    fi
                    if [[ ${v_rm_dn_num} = 0 ]];then
                       echo "Msg: Remove DataNode ${rm_dn_ip} success."
                       break
             
                    fi
                    if [[ ${v_loop} -gt 600 ]];then
                       let fail_flag++
                       echo "Msg:Removing status is not right."
                       let remove_suc_time=0
                       break
                    fi
		done
        fi
...
last_dn_ip3=`tail -2 ${nodeinfo_dir}/datanode.txt|tail -1`
remove_datanode ${last_dn_ip2} ${last_dn_ip2} "success"
# check data consistent
if [[ ${remove_suc_time} -gt 0 ]];then
...
while read line<&3
do
   ssh ${u_name}@${line} "sudo gunzip ${db_dir}/logs/log-datanode-*"
   v_npe=`ssh ${u_name}@${line} "grep NullPointer ${db_dir}/logs/*datanode*all*|wc -l"`
   if [[ ${v_npe} -gt 0 ]];then
       let fail_flag++
       echo "${line} NullPointer : ${v_npe}"
   fi
done
exec 3<${nodeinfo_dir}/confignode.txt
while read line<&3
do
   ssh ${u_name}@${line} "sudo gunzip ${db_dir}/logs/log-confignode-*"
