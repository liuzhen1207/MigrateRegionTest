...
stop_benchmarks() {
  ...
  for workload in tree_nonaligned tree_aligned tree_aligned_temp table
  do
    [[ -f "${bm_log_root}/${workload}.pid" ]] || continue
...
collect_v2_1215_log_evidence() {
  local evidence_file="${cur_dir}/v2_1215_sync_readonly.log"
  local dn_ip
  local matched=0
  local node_evidence="${cur_dir}/v2_1215_node_evidence.tmp"
  ...
  do
    ...
    if [[ -s "${node_evidence}" ]]; then
      echo "===== sender ${dn_ip}, ReadOnly peer ${v2_1215_adding_ip} =====" >> "${evidence_file}"
      cat "${node_evidence}" >> "${evidence_file}"
      matched=1
    fi
...
mark_v2_1215_detected() {
  local reason

  [[ ${v2_1215_detected} -eq 1 ]] && return 0
  reason="V2-1215: remove ${rm_target_ip} is stuck because Adding peer ${v2_1215_adding_ip} is ReadOnly and rejects IoTConsensus log sync"
  log "${reason}"
  append_warn "${reason}"
  let fail_flag++
  let rm_fail_flag++
...
check_v2_1215_logs() {
  [[ -n "${v2_1215_adding_ip}" ]] || return 0
  if collect_v2_1215_log_evidence; then
    mark_v2_1215_detected
    log "V2-1215 log evidence saved to ${cur_dir}/v2_1215_sync_readonly.log"
    return 1
  fi
  return 0
}

restore_adding_peer_running() {
  [[ -n "${v2_1215_adding_ip}" ]] || return 0

  log "restore Adding peer ${v2_1215_adding_ip} to Running"
  run_cli_sql "${v2_1215_adding_ip}" tree "set system to running on local;" "${cur_dir}/set_adding_running.out" 300
  if ! wait_datanode_status "${query_ip}" "${v2_1215_adding_ip}" "Running" 300; then
    log "Adding peer ${v2_1215_adding_ip} did not return to Running"
    append_warn "Adding peer ${v2_1215_adding_ip} did not return to Running"
    let fail_flag++
    return 1
  fi
  return 0
...
remove_datanode_and_make_adding_peer_readonly() {
  local target_ip=$1
  local rm_dn_id
  local adding_region_id=""
  local adding_ip=""
  local start_time
  local stable_zero=0
  local active_num
  ...
  run_cli_sql "${query_ip}" tree "show datanodes;" "${cur_dir}/show_datanodes.out" 3600
  rm_dn_id=$(grep "${target_ip}|" "${cur_dir}/show_datanodes.out" | awk -F '|' '{gsub(" ","",$2);print $2}' | tail -1)
  if [[ -z "${rm_dn_id}" ]]; then
    log "failed to locate remove target id for ${target_ip}"
    let fail_flag++
    return 1
  fi
  if ! wait_datanode_status "${query_ip}" "${target_ip}" "Running" 300; then
    log "remove target ${target_ip} is not Running"
    let fail_flag++
    return 1
  fi

  run_cli_sql "${target_ip}" tree "remove datanode ${rm_dn_id};" "${cur_dir}/remove_datanode.out" 3600
  if ! grep -qi 'successfully' "${cur_dir}/remove_datanode.out"; then
    log "remove datanode submit output is not success"
    cat "${cur_dir}/remove_datanode.out"
    let fail_flag++
    return 1
  ...
  # Catch the first migration destination in Adding state. Use a bounded wait
  # so a fast migration or an abnormal procedure cannot hang the testcase.
  start_time=$(date +%s)
  while (( $(date +%s) - start_time <= 120 ))
  do
    run_cli_sql "${query_ip}" table "show data regions;" "${cur_dir}/show_data_regions_table.out" 3600
    adding_region_id=$(grep 'Adding' "${cur_dir}/show_data_regions_table.out" | head -1 | awk -F '|' '{gsub(" ","",$2);print $2}')
    adding_ip=$(grep 'Adding' "${cur_dir}/show_data_regions_table.out" | head -1 | awk -F '|' '{gsub(" ","",$9);print $9}')
    if [[ -z "${adding_region_id}" || -z "${adding_ip}" ]]; then
      run_cli_sql "${query_ip}" tree "show data regions;" "${cur_dir}/show_data_regions_tree.out" 3600
      adding_region_id=$(grep 'Adding' "${cur_dir}/show_data_regions_tree.out" | head -1 | awk -F '|' '{gsub(" ","",$2);print $2}')
      adding_ip=$(grep 'Adding' "${cur_dir}/show_data_regions_tree.out" | head -1 | awk -F '|' '{gsub(" ","",$9);print $9}')
    fi
    [[ -n "${adding_region_id}" && -n "${adding_ip}" ]] && break
    sleep 1
  done
  if [[ -z "${adding_region_id}" || -z "${adding_ip}" ]]; then
    log "no Adding DataRegion was observed within 120 seconds"
    let fail_flag++
    return 1
  fi
  if [[ "${adding_ip}" = "${target_ip}" ]]; then
    log "Adding replica unexpectedly points to remove target ${target_ip}"
    let fail_flag++
    return 1
  fi

  log "set Adding replica ${adding_region_id} on ${adding_ip} to ReadOnly"
  rm_target_ip="${target_ip}"
  v2_1215_adding_ip="${adding_ip}"
  v2_1215_adding_region_id="${adding_region_id}"
  run_cli_sql "${adding_ip}" tree "set system to readonly on local;" "${cur_dir}/set_adding_readonly.out" 300
  if ! wait_datanode_status "${query_ip}" "${adding_ip}" "ReadOnly" 300; then
    log "Adding replica DataNode ${adding_ip} did not become ReadOnly"
    let fail_flag++
    return 1
  fi

  # The procedure may finish by removing the target or roll back and keep it
  # Running. Both are valid here; a lasting Adding/Removing state is not.
  start_time=$(date +%s)
  while (( $(date +%s) - start_time <= 3600 ))
  do
    run_cli_sql "${query_ip}" table "show regions;" "${cur_dir}/show_regions_table.out" 3600
    run_cli_sql "${query_ip}" tree "show regions;" "${cur_dir}/show_regions_tree.out" 3600
    run_cli_sql "${query_ip}" tree "show datanodes;" "${cur_dir}/show_datanodes.out" 3600
    active_num=$(grep -Ec 'Adding|Removing' "${cur_dir}/show_regions_table.out")
    active_num=$((active_num + $(grep -Ec 'Adding|Removing' "${cur_dir}/show_regions_tree.out")))
    target_num=$(grep "${target_ip}|" "${cur_dir}/show_datanodes.out" | wc -l)
    target_status=$(grep "${target_ip}|" "${cur_dir}/show_datanodes.out" | awk -F '|' '{gsub(" ","",$3);print $3}' | tail -1)

    if check_v2_1215_logs; then
      :
    else
      return 1
    fi
    ...
    if [[ ${stable_zero} -ge 3 ]]; then
      if [[ ${target_num} -eq 0 ]]; then
        ...
        log "remove settled with unexpected target status=${target_status}"
        let fail_flag++
        return 1
      fi
      break
  ...
  if [[ -z "${outcome}" ]]; then
    log "remove stayed in Adding/Removing state for more than 3600 seconds"
    let fail_flag++
    return 1
  fi

  if [[ "${outcome}" = "removed" ]]; then
    jps_num=$(ssh "${u_name}@${target_ip}" "source /etc/profile; sudo jps -l | grep -c 'DataNode' || true")
    tsfile_num=$(ssh "${u_name}@${target_ip}" "sudo find '${db_dir}/data' -name '*.tsfile' -type f | wc -l")
    if [[ ${jps_num} -ne 0 || ${tsfile_num} -ne 0 ]]; then
      log "removed target ${target_ip} still has process=${jps_num}, tsfiles=${tsfile_num}"
      let fail_flag++
      return 1
    fi
  fi
  log "Adding-ReadOnly remove settled: target=${target_ip}, adding_ip=${adding_ip}, outcome=${outcome}"
  return 0
}
...
testcase() {
  ...
  target_ip=$(tail -2 "${nodeinfo_dir}/datanode.txt" | head -1)
  remove_datanode_and_make_adding_peer_readonly "${target_ip}" || remove_rc=$?
  restore_adding_peer_running || true
  check_v2_1215_logs || true
  if [[ ${remove_rc} -ne 0 || ${v2_1215_detected} -eq 1 ]]; then
    stop_benchmarks
    return 1
  fi
