...
backup_dir_on_cn_dn_host=/data/iotdb/autotest_backup/3db_test_data_IoTV1_remove
...
#        v_rm_datanode_id2=`${cli_dir}/sbin/start-cli.sh -h ${query_ip} -e "show datanodes;"|grep ${exec_rm_ip}|awk -F '|' '{gsub(" ","");print $2}'`
# copy remove-datanode.sh
# 70.30 has  6|SchemaRegion|Unknown|    root.__system| ,remove will fail        
...
# check remove-dn regions 
        ...
        do
                      ...
                      if [[ ${v_rm_suc} = 0 ]];then
                         v_other_dn=`cat ${cur_dir}/show_regions.out|grep "${query_ip}|"|wc -l`
                   ...
                   let loop++
                   if [[ ${loop} -gt 360 ]];then
                      echo "ERROR : remove ${rm_dn_ip} 1hour fail."
                      let fail_flag++
                      return 1
                   fi
        done
	...
	do
		   ...
		   let loop++
		   if [[ ${loop} -gt 60 ]];then
		      echo "ERROR : remove ${rm_dn_ip} fail."
		      let fail_flag++
                      rm_flag=fail
		      return 1
		   fi
...
#        for i in {1..10}
#        do
#            v_running_num=`${cli_dir}/sbin/start-cli.sh -h ${query_ip} -e "show datanodes;"|grep -i running|wc -l`
#            if [[ ${v_running_num} != ${dn_num} ]];then
#               let fail_flag++
#            fi
#            sleep 1
#        done
#        if [ "${db_dir}" != "" ]; then
#           ssh ${u_name}@${exec_rm_ip} "rm -rf ${db_dir}/tmp"
#        fi
#        v_loop=0
#        while true 
#        do
#            v_running_num=`${cli_dir}/sbin/start-cli.sh -h ${query_ip} -e "show datanodes;"|grep "${rm_dn_ip}|" |wc -l`
#            if [[ ${v_running_num} -gt 0 ]];then
#               sleep 1
#            else
#               break 
#            fi
#            let v_loop++
#            if [[ ${v_loop} -gt 1800 ]];then
#               let fail_flag++
#               break
#            fi 
#        done
        ...
        if [[ ${v_rm_suc} -gt 0 ]];then
		   ...
		   if [[ ${v_pid} -gt 0 ]];then
                   ...
                   if [[ ${v_loop} -gt 120 ]];then
                      ssh ${u_name}@${rm_dn_ip} "sudo jstack -l ${v_pid} > ${db_dir}/stop_failed_jstack_${v_pid}_${test_begin_sec}.out"
		      let fail_flag++
                      return 1
                      break
                   fi
           ...
           # rm -rf datanode
           if [[ "${db_dir}" != "" ]];then
              ssh ${u_name}@${rm_dn_ip} "sudo rm -rf ${db_dir}/data/datanode"
           fi
#           # start removed datanode
#           ssh ${u_name}@${rm_dn_ip} "sudo ${db_dir}/sbin/start-datanode.sh -H ${db_dir}/${test_begin_sec}_dn.hprof >/dev/null 2>&1 &"
#           v_loop=0
#		while true
#		do
#		    v_running_num=`${cli_dir}/sbin/start-cli.sh -h ${query_ip} -e "show datanodes;"|grep "${rm_dn_ip}|" |grep -i running|wc -l`
#		    if [[ ${v_running_num} = 0 ]];then
#		       sleep 1
#		    else
#		       break
#		    fi
...
#		done
        ...
        while true
        do
                      v_rm_suc=`${cli_dir}/sbin/start-cli.sh -h ${query_ip} -e "show datanodes;"|grep "${rm_dn_ip}|"|grep -i removing|wc -l`
                      if [[ ${v_rm_suc} = 0 ]];then
                         sleep 1 
                      else
                         let fail_flag++
                  ...
                  v_rm_fail_msg=`grep "Can't remove datanode due to the limit of replication factor" ${cur_dir}/rm_cmd_res.out|wc -l` 
                  if [[ ${v_rm_fail_msg} = 0 ]];then
                     let fail_flag++
                  fi
        done
        v_rm_suc=`${cli_dir}/sbin/start-cli.sh -h ${query_ip} -e "show datanodes;"|grep "${rm_dn_ip}|"|grep -i running|wc -l`
        if [[ ${v_rm_suc} = 0 ]];then
           let fail_flag++
           echo "ERROR: ${rm_dn_ip} is not Running."
        fi
...
#        while true
#        do
#            v_running_num=`${cli_dir}/sbin/start-cli.sh -h ${query_ip} -e "show datanodes;"|grep ${exec_rm_ip} |grep -i running|wc -l`
#            if [[ ${v_running_num} -gt 0 ]];then
#               sleep 1
#            else
#               break
...
remove_datanode ${last_dn_ip} ${last_dn_ip2} "success"
