...
sed -i "s/^PASSWORD=.*/PASSWORD=${bm_root_pw}/g" ${bm_dir}/lt_10type_user_no_ssl/conf*/config.properties
   ...
   nohup sh -x ${bm_dir}/benchmark.sh -cf ${bm_dir}/lt_10type_user_no_ssl/conf1 >${bm_dir}/${v_t}_tc11_bm1.out &
   nohup sh -x ${bm_dir}/benchmark.sh -cf ${bm_dir}/lt_10type_user_no_ssl/conf2 >${bm_dir}/${v_t}_tc11_bm2.out &
   sleep 30
...
# extend region
   ...
   check_res "successfully" 1 "${SCRIPT_NAME}"
   check_res2 "successfully submitted: 0" "Target DataNode ${v_rm_id} is not in Running status" 1 "${SCRIPT_NAME}"
   cat ${cur_dir}/tmp.out
   fi
   done
   ...
   wait_bm_finish 36000 "${bm_dir}/${v_t}_tc11_bm1.out" "${bm_dir}/${v_t}_tc11_bm2.out"
...
# expect remove dn succ
${cli_dir}/sbin/start-cli.sh -u ${db_sys_admin} ${ssl_str} -h ${query_ip} -e "show datanodes;">${cur_dir}/tmp.out
   check_res "${v_rm_ip}|" 0 "${SCRIPT_NAME}"
