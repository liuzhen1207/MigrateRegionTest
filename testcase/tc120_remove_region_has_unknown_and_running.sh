...
function check_rm_dn_regions()
{
   ...
   local flag_no_remove=0
        ...
        do
                   ...
                   if [[ ${loop} -gt 360 ]];then
                      echo "ERROR : remove ${rm_dn_ip} 1 hour fail."
                      let fail_flag++
                      return 1
                   fi
        done
...
# stop 1 dn
stop_dn ${last_dn_ip2}
# remove 1 dn its region has unknown and running ,exp succ,but currently fail 
remove_1datanode ${last_dn_ip} ${last_dn_ip2} "fail"
if [[ "${rm_flag}" = "success" ]];then
check_rm_dn_regions ${last_dn_ip}
#check_rm_dn_regions ${last_dn_ip2}
check_rm_dn_status ${last_dn_ip}
#check_rm_dn_status ${last_dn_ip2}
