...
cp -rp ${cur_dir}/rm_dn1_regionid.txt ${cur_dir}/pre_mig_id.txt
...
do
	  # get to nodeid
	  v_mig_to_id=`grep "^${v_mig_id}," ${cur_dir}/rm_all_regionid_nodeid.txt|grep -v "^${v_mig_id},${v_mig_from_id}"|head -1|awk -F ',' '{print $2}'`
...
#	stop_dn ${last_dn_ip2}
	remove_1datanode ${last_dn_ip} ${last_dn_ip2} "success"
        ...
        v_mig_fail_num=`grep "Submit RegionMigrateProcedure failed" ${cur_dir}/remove_mig.out |wc -l`
        if [[ ${v_mig_fail_num} -lt ${v_mig_num} ]];then
           let fail_flag++
        fi
