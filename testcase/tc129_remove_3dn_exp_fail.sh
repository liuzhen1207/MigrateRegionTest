...
function remove_1datanode()
{
   ...
   if [[ "${rm_flag}" = "success" ]];then
        ...
        v_rm_fail=`cat ${cur_dir}/rm_cmd_res.out|grep "code:1003, message:Can't remove datanode due to the limit of replication factor"|wc -l`
        if [[ ${v_rm_fail} = 0 ]];then
           let fail_flag++
        fi
   fi
...
function remove_3datanode()
{
        ...
        v_rm_datanode_id=`${cli_dir}/sbin/start-cli.sh -h ${query_ip} -e "show datanodes;"|grep "${rm_dn_ip}|"|awk -F '|' '{gsub(" ","");print $2}'`
        v_rm_datanode_id2=`${cli_dir}/sbin/start-cli.sh -h ${query_ip} -e "show datanodes;"|grep "${rm_dn_ip2}|"|awk -F '|' '{gsub(" ","");print $2}'`
        v_rm_datanode_id3=`${cli_dir}/sbin/start-cli.sh -h ${query_ip} -e "show datanodes;"|grep "${rm_dn_ip3}|"|awk -F '|' '{gsub(" ","");print $2}'`

        ssh ${u_name}@${exec_rm_ip} "sudo ${db_dir}/sbin/remove-datanode.sh  ${v_rm_datanode_id} ${v_rm_datanode_id2} ${v_rm_datanode_id3}">${cur_dir}/rm_cmd_res.out 2>>${cur_dir}/rm_cmd_res.out
   if [[ "${rm_flag}" = "success" ]];then
        v_rm_fail=`cat ${cur_dir}/rm_cmd_res.out|grep "Submit remove-datanode request successfully"|wc -l`
        if [[ ${v_rm_fail} = 0 ]];then
           let rm_flag=fail
...
#done
#	stop_dn ${last_dn_ip2}
	remove_3datanode ${last_dn_ip} ${last_dn_ip2} ${last_dn_ip3} "fail"
