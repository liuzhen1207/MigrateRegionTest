...
# exec migrate
  ...
  while true
   ...
   v_mig_res=`${cli_dir}/sbin/start-cli.sh -h ${query_ip} -e "MIGRATE REGION ${v_mig_id} FROM ${v_mig_from_dn_id} TO ${v_mig_to_dn_id};"|grep "IoTDBSQLException: 900: Target DataNode ${v_mig_to_dn_id} is not in Running status"|wc -l`
   if [[ ${v_mig_res} = 0 ]];then
      let fail_flag++
   fi
