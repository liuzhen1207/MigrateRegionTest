...
#done
#	stop_dn ${last_dn_ip2}
	remove_1datanode ${last_dn_ip} ${last_dn_ip2} "fail"
#        sleep 5
#        kill9_dn ${last_dn_ip}
...
#while true
#do
#   v_add_num=`${cli_dir}/sbin/start-cli.sh -h ${query_ip} -e "show regions;"|grep -i adding|wc -l`
#   v_removing_num=`${cli_dir}/sbin/start-cli.sh -h ${query_ip} -e "show regions;"|grep -i removing|wc -l`
#   v_mig_num=$((v_add_num+v_removing_num))
#   if [[ ${v_mig_num} = 0 ]];then
#      let loop++
#   fi
#   sleep 1
