...
# exec migrate
   ...
   v_stop_ip=`grep "^${v_mig_to_dn_id}," ${cur_dir}/all_dn_id_ip.txt|awk -F ',' '{print $2}'`
   ...
   sleep 5 
# stop dest dn
   ssh ${u_name}@${v_stop_ip} "sudo ${db_dir}/sbin/stop-datanode.sh"
   sleep 2
  while true
   do
      v_check_stop=`${cli_dir}/sbin/start-cli.sh -h ${query_ip} -e "show datanodes"|grep "${v_stop_ip}"|grep -i unknown|wc -l`
   ...
   do
      ...
      v_mig_suc=`ssh ${u_name}@${v_cn_leader_ip} "grep \"RegionMigrateProcedure will not continue\" ${db_dir}/logs/*confignode*all.log|wc -l"`
      v_mig_suc_2=`ssh ${u_name}@${v_cn_leader_ip} "grep \"\[MigrateRegion\] success\" ${db_dir}/logs/*confignode*all.log|wc -l"`
      if [[ ${v_mig_suc} -gt 0 ]] || [[ ${v_mig_suc_2} -gt 0 ]];then
         break
      else
         sleep 5
      fi
...
# restart unknown dn
v_start_time=`date +%s`
ssh ${u_name}@${v_stop_ip} "source /etc/profile;sudo ${db_dir}/sbin/start-datanode.sh -H ${db_dir}/dn_${v_start_time}_heapdump.hprof > /dev/null 2>&1 &"
while true
do
sleep 1
      v_restart_unknown=`${cli_dir}/sbin/start-cli.sh -h ${query_ip} -e "show cluster"|grep -i datanode|grep ${v_stop_ip}|grep -i Running|wc -l`
...
v_diff=`diff ${cur_dir}/q_act.out ${cur_dir}/q_exp.out|grep root|wc -l`
if [[ ${v_diff} -gt 0 ]];then
let fail_flag++
fi
>ignore_dn_list.txt
