...
function stop_cn()
{
   local rm_dn_ip=$1
        ...
        while true
        do
           v_unknown=`${cli_dir}/sbin/start-cli.sh -h ${query_ip} -e "show confignodes;"|grep "${rm_dn_ip}|"|grep -i unknown|wc -l`
           if [[ ${v_unknown} -gt 0 ]];then
              break
           else
              sleep 2
        ...
        do
                      ...
                      if [[ ${v_rm_suc} = 0 ]];then
                         ...
                         if [[ ${v_other_dn} -gt 0 ]];then
                            ...
                            v_cur_sec=`date +%s`
                            v_fail_str=`grep "Fail to connect to any config node" ${cur_dir}/show_regions.out|wc -l`
                            if [[ ${v_fail_str} -gt 0 ]];then
                               let show_fail_num++
                            fi 
...
#	stop_dn ${last_dn_ip2}
	remove_1datanode ${last_dn_ip} ${last_dn_ip2} "success"
        sleep 2
        ${cli_dir}/sbin/start-cli.sh -h ${query_ip} -e "show confignodes;"|grep Follower|awk -F "|" '{gsub(" ","");print $4}'>${cur_dir}/follower.txt
        v_cn_leader=`head -1 ${cur_dir}/follower.txt`
        if [[ "${v_cn_leader}" != "" ]];then
           kill9_cn "${v_cn_leader}"
        fi
        v_cn_leader2=`tail -1 ${cur_dir}/follower.txt`
        if [[ "${v_cn_leader2}" != "" ]];then
           kill9_cn "${v_cn_leader2}"
        fi
...
#        fi
v_unknown_num=`${cli_dir}/sbin/start-cli.sh -h ${query_ip} -e "show regions;"|grep Unknown|wc -l`
if [[ ${v_unknown_num} -gt 0 ]];then
let fail_flag++
fi

v_unknown_num=`${cli_dir}/sbin/start-cli.sh -h ${query_ip} -e "show cluster;"|grep Unknown|wc -l`
if [[ ${v_unknown_num} -gt 0 ]];then
let fail_flag++
fi
