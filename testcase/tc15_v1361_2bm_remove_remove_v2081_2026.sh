...
sed -i "s/^PASSWORD=.*/PASSWORD=${bm_root_pw}/g" ${bm_dir}/lt_10type_user_no_ssl/conf*/config.properties
   ...
   bm_res1="${bm_dir}/${v_t}_tc15_bm1.out"
   bm_res2="${bm_dir}/${v_t}_tc15_bm2.out"
...
>${cur_dir}/region.txt
   ...
   # REMOVE REGION is asynchronous. The duplicate REMOVE failure above is
   # expected while the first procedures are running; wait for the requested
   # regions to disappear from the target before judging the final result.
