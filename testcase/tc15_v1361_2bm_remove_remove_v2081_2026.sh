...
sed -i "s/^PASSWORD=.*/PASSWORD=${bm_root_pw}/g" ${bm_dir}/lt_10type_user_no_ssl/conf*/config.properties
   ...
   nohup sh -x ${bm_dir}/benchmark.sh -cf ${bm_dir}/lt_10type_user_no_ssl/conf1 >${bm_dir}/${v_t}_tc15_bm1.out &
   nohup sh -x ${bm_dir}/benchmark.sh -cf ${bm_dir}/lt_10type_user_no_ssl/conf2 >${bm_dir}/${v_t}_tc15_bm2.out &
   sleep 60
# extend region
...
>${cur_dir}/region.txt
   ...
   wait_bm_finish 36000 "${bm_dir}/${v_t}_tc15_bm1.out" "${bm_dir}/${v_t}_tc15_bm2.out"
