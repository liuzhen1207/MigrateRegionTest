...
sed -i "s/^PASSWORD=.*/PASSWORD=${bm_root_pw}/g" ${bm_dir}/lt_10type_user_no_ssl/conf*/config.properties
   ...
   nohup sh -x ${bm_dir}/benchmark.sh -cf ${bm_dir}/lt_10type_user_no_ssl/conf1 >${bm_dir}/${v_t}_tc16_bm1.out &
   nohup sh -x ${bm_dir}/benchmark.sh -cf ${bm_dir}/lt_10type_user_no_ssl/conf2 >${bm_dir}/${v_t}_tc16_bm2.out &
   sleep 60
# extend region
   ...
   ${cli_dir}/sbin/start-cli.sh -h ${query_ip} -u ${db_sys_admin} ${ssl_str} -e  'show datanodes;'|grep -v ${v_rm_ip}|grep Running|awk -F '|' '{gsub(" ","");print $2","}'>${cur_dir}/all_dn_id.txt
   v_mig_sql="" 
>${cur_dir}/region.txt
   ...
   ${cli_dir}/sbin/start-cli.sh -h ${query_ip} -u ${db_sys_admin} ${ssl_str} -e  'show regions'|grep "^| *${line}|"|awk -F '|' '{gsub(" ","");print $8","}'>${cur_dir}/region_dn_id.txt
   exec 4<${cur_dir}/all_dn_id.txt
   while read id<&4
   do
       v_exist_flag=`grep ${id} ${cur_dir}/region_dn_id.txt|wc -l`
       v_to_id=`echo ${id}|awk -F ',' '{print $1}'`
       if [[ ${v_exist_flag} = 0 ]];then
          v_mig_sql="${v_mig_sql}MIGRATE REGION ${line} FROM ${v_rm_id} TO ${v_to_id};"
          break 
       fi
   done
   fi
   ...
   ${cli_dir}/sbin/start-cli.sh -h ${query_ip} -e "EXTEND REGION ${v_remove_list}  TO ${v_rm_id};">${cur_dir}/tmp.out 
   check_res "successfully" 1 "${SCRIPT_NAME}"
   cat ${cur_dir}/tmp.out
   echo "MIGRATE REGION TIME: $(date "+%Y-%m-%d %H:%M:%S")"
if [[ -n "$v_mig_sql" ]];then
   ${cli_dir}/sbin/start-cli.sh -h ${query_ip} -e "${v_mig_sql}">${cur_dir}/tmp.out 
   check_res2 "failed to submit:" "has some other region operation procedures in progress" 1 "${SCRIPT_NAME}"
   cat ${cur_dir}/tmp.out
fi
   wait_bm_finish 36000 "${bm_dir}/${v_t}_tc16_bm1.out" "${bm_dir}/${v_t}_tc16_bm2.out"
