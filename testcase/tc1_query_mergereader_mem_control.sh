...
{
        ${cli_dir}/sbin/start-cli.sh -h ${query_ip} -e "select s1,s2,s3,s4,s5,s6 into root.db.d1 from root.db.d2" &
        ${cli_dir}/sbin/start-cli.sh -h ${query_ip} -e "select s1,s2,s3,s4,s5,s6 into root.db.d2 from root.db.d1" &
...
function test1()
{
  ...
  v_inc=0
     for j in {1..100}
     do
        ${cli_dir}/sbin/start-cli.sh -h ${query_ip} -e "insert into root.db.d1(time,s1,s2,s3,s4,s5,s6,s7,s8,s9,s10) values(${j},true,${v_inc},'hello1',1,1,1.9,now(),'Hong Kong',X'486f6e67204b6f6e6720426c6f6221','1997-07-01');" &
        ${cli_dir}/sbin/start-cli.sh -h ${query_ip} -e "insert into root.db.d2(time,s1,s2,s3,s4,s5,s6,s7,s8,s9,s10) values(${j},true,${v_inc},'hello1',1,1,1.9,now(),'Hong Kong',X'486f6e67204b6f6e6720426c6f6221','1997-07-01');" &
        let v_inc++
     done
     ${cli_dir}/sbin/start-cli.sh -h ${query_ip} -e "flush;"
  for i in {1..1000}
  ...
  do
     ...
     v_error=`ssh ${u_name}@${node} "grep ERROR ${db_dir}/logs/*datanode*all*"|wc -l"`
