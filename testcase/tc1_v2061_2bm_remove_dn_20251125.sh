...
#start 2bm
   ...
   sed -i "s/^HOST=.*/HOST=${v_host}/g" ${bm_dir}/lt_10type_user_no_ssl_v200/conf*/config.properties
   sed -i "s/LOOP=.*/LOOP=10000/g" ${bm_dir}/lt_10type_user_no_ssl_v200/conf*/config.properties
   nohup sh -x ${bm_dir}/benchmark.sh -cf ${bm_dir}/lt_10type_user_no_ssl_v200/conf1 >${bm_dir}/${v_t}_tc1_bm1.out &
   nohup sh -x ${bm_dir}/benchmark.sh -cf ${bm_dir}/lt_10type_user_no_ssl_v200/conf2 >${bm_dir}/${v_t}_tc1_bm2.out &
   sleep 30
   ${cli_dir}/sbin/start-cli.sh -u ${db_sys_admin} ${ssl_str} -h ${query_ip} -e "remove datanode ${v_rm_id};">${cur_dir}/tmp.out
   check_res "success" 1 "${SCRIPT_NAME}"
# wait bm finish
   wait_bm_finish 36000 "${bm_dir}/${v_t}_tc1_bm1.out" "${bm_dir}/${v_t}_tc1_bm2.out" 
