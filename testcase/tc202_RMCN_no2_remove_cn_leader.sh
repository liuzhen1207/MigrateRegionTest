...
function insert_data()
{
  ...
  ${cli_dir}/sbin/start-cli.sh -h ${query_ip} -e "create timeseries root.db.d1.s7 timestamp encoding=PLAIN;"
  ${cli_dir}/sbin/start-cli.sh -h ${query_ip} -e "create timeseries root.db.d1.s8 string encoding=PLAIN;"
  ${cli_dir}/sbin/start-cli.sh -h ${query_ip} -e "create timeseries root.db.d1.s9 blob encoding=PLAIN;"
  ${cli_dir}/sbin/start-cli.sh -h ${query_ip} -e "create timeseries root.db.d1.s10 date encoding=PLAIN;"
  ${cli_dir}/sbin/start-cli.sh -h ${query_ip} -e "CREATE ALIGNED TIMESERIES root.db.d2(s1 BOOLEAN encoding=PLAIN, s2 FLOAT encoding=RLE,s3 TEXT encoding=PLAIN,s4 INT32 encoding=PLAIN,s5 INT64 encoding=PLAIN,s6 DOUBLE encoding=PLAIN,s7 timestamp,s8 string,s9 blob,s10 date) ;"
  ${cli_dir}/sbin/start-cli.sh -h ${query_ip} -e "CREATE ALIGNED TIMESERIES root.db.d3(s1 BOOLEAN encoding=PLAIN, s2 FLOAT encoding=RLE,s3 TEXT encoding=PLAIN,s4 INT32 encoding=PLAIN,s5 INT64 encoding=PLAIN,s6 DOUBLE encoding=PLAIN,s7 timestamp,s8 string,s9 blob,s10 date) ;"
  ${cli_dir}/sbin/start-cli.sh -h ${query_ip} -e "CREATE ALIGNED TIMESERIES root.db.d4(s1 BOOLEAN encoding=PLAIN, s2 FLOAT encoding=RLE,s3 TEXT encoding=PLAIN,s4 INT32 encoding=PLAIN,s5 INT64 encoding=PLAIN,s6 DOUBLE encoding=PLAIN,s7 timestamp,s8 string,s9 blob,s10 date) ;"
  ${cli_dir}/sbin/start-cli.sh -h ${query_ip} -e "CREATE ALIGNED TIMESERIES root.db.d2(s1 BOOLEAN encoding=PLAIN, s2 FLOAT encoding=RLE,s3 TEXT encoding=PLAIN,s4 INT32 encoding=PLAIN,s5 INT64 encoding=PLAIN,s6 DOUBLE encoding=PLAIN,s7 timestamp,s8 string,s9 blob,s10 date) ;"

        ${cli_dir}/sbin/start-cli.sh -h ${query_ip} -e "select s1,s2,s3,s4,s5,s6,s7,s8,s9,s10 into root.db.d1 from root.db.d2" &
        ${cli_dir}/sbin/start-cli.sh -h ${query_ip} -e "select s1,s2,s3,s4,s5,s6,s7,s8,s9,s10 into root.db.d2 from root.db.d1" &
...
#        v_rm_datanode_id=`${cli_dir}/sbin/start-cli.sh -h ${query_ip} -e "show datanodes;"|grep ${rm_dn_ip}|awk -F '|' '{gsub(" ","");print $2}'`
        v_rm_datanode_id=6667
        v_rm_res=`ssh ${u_name}@${exec_rm_ip} "sudo ${db_dir}/sbin/start-cli.sh -h ${query_ip} -e \"remove confignode ${v_rm_datanode_id}\""`
        echo "${v_rm_res}"
        v_rm_succ=`echo "${v_rm_res}"|grep "no detailed failure reason in QueryStateMachine"|wc -l`
        v_rm_msg=`echo "${v_rm_res}"|grep "The ConfigNode to be removed is not in the cluster, or the input format is incorrect" |wc -l`
        if [[ ${v_rm_succ} = 1 ]];then
           let fail_flag++
        fi
        if [[ ${v_rm_msg} = 0 ]];then
        ...
        v_rm_datanode_id=null
        v_rm_res=`ssh ${u_name}@${exec_rm_ip} "sudo ${db_dir}/sbin/start-cli.sh -h ${query_ip} -e \"remove confignode ${v_rm_datanode_id}\""`
        echo "${v_rm_res}"
        v_rm_succ=`echo "${v_rm_res}"|grep "no detailed failure reason in QueryStateMachine"|wc -l`
        v_rm_msg=`echo "${v_rm_res}"|grep "Error occurred while parsing SQL"|wc -l`
        if [[ ${v_rm_succ} = 1 ]];then
           let fail_flag++
        fi
        if [[ ${v_rm_msg} = 0 ]];then
