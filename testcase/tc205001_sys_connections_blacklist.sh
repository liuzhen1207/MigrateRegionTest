...
# set black list 
   ${cli_dir}/sbin/start-cli.sh -h ${query_ip} -u ${db_sys_admin} ${ssl_str} -sql_dialect table -e "show cluster;set configuration black_ip_list='${LOCAL_IP}';load configuration;show cluster;">${cur_dir}/tmp.out
   check_res "success" 2 "${SCRIPT_NAME}" 
   ${cli_dir}/sbin/start-cli.sh -h ${query_ip} -u ${db_sys_admin} ${ssl_str} -sql_dialect table -e  "select * from information_schema.connections;">${cur_dir}/tmp.out
   ...
   check_res2 "${LOCAL_IP}" "lily" 100 "${SCRIPT_NAME}"
   ${cli_dir}/sbin/start-cli.sh -h ${query_ip} -u ${db_sys_admin} ${ssl_str} -sql_dialect table -e "show cluster;set configuration black_ip_list='192.168.10.71';load configuration;show cluster;">${cur_dir}/tmp.out
   check_res "whether the client IP address is in the whitelist or not in the blacklist"  1 "${SCRIPT_NAME}"
   ssh ${u_name}@${query_ip} "${db_dir}/sbin/start-cli.sh -h ${query_ip} -u ${db_sys_admin} ${ssl_str} -sql_dialect table -e  \"show cluster;set configuration black_ip_list='192.168.10.71';load configuration;show cluster;\"">${cur_dir}/tmp.out
   check_res "success" 2 "${SCRIPT_NAME}"
   ${cli_dir}/sbin/start-cli.sh -h ${query_ip} -u ${db_sys_admin} ${ssl_str} -sql_dialect table -e  "select * from information_schema.connections;">${cur_dir}/tmp.out
   check_res2 "${LOCAL_IP}" "lily" 100 "${SCRIPT_NAME}"
