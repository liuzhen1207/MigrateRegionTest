...
# drop user
   echo "rename user lily at `date "+%Y-%m-%d %H:%M:%S"`" 
   ${cli_dir}/sbin/start-cli.sh -h ${query_ip} -u ${db_sys_admin} ${ssl_str} -sql_dialect table -e "alter user lily rename to lucy;">${cur_dir}/tmp.out
   check_res "success" 1 "${SCRIPT_NAME}"
   sleep 2 
   ${cli_dir}/sbin/start-cli.sh -h ${query_ip} -u ${db_sys_admin} ${ssl_str} -sql_dialect table -e  "select * from information_schema.connections;">${cur_dir}/tmp.out
   check_res "lily" 0 "${SCRIPT_NAME}"
   check_res "lucy" 0 "${SCRIPT_NAME}"
#   check_res "whether the client IP address is in the whitelist or not in the blacklist"  1 "${SCRIPT_NAME}"
   ...
   check_res "lily" 0 "${SCRIPT_NAME}"
   ${cli_dir}/sbin/start-cli.sh -h ${query_ip} -u ${db_sys_admin} ${ssl_str} -sql_dialect table -e "alter user lucy rename to lily;">${cur_dir}/tmp.out
   check_res "success"  1 "${SCRIPT_NAME}"
   ${cli_dir}/sbin/start-cli.sh -h ${query_ip} -u ${db_sys_admin} ${ssl_str} -sql_dialect table -e  "select * from information_schema.connections;">${cur_dir}/tmp.out
   check_res "lily" 0 "${SCRIPT_NAME}"
   jps|grep App|awk '{print "kill -9 "$1}'|sh
