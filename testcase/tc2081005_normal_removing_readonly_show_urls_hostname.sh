...
ssh ${u_name}@${line} "sudo sh -c \"sync; echo 3 > /proc/sys/vm/drop_caches\"";
   ...
   sh -x ${prepare_env_dir}/start_cluster_hostname.sh "1" "${total_node_num}"
...
check_res "tcho" ${dn_num} "${SCRIPT_NAME}"
cat ${cur_dir}/tmp.out
...
cp -rp ${nodeinfo_dir}/total_confignode.txt  ${nodeinfo_dir}/confignode.txt
}
clean_env
start_db
remove_dn
