...
function mig_region()
{
   ...
   v_exp_error=`${cli_dir}/sbin/start-cli.sh -h ${query_ip} -e "MIGRATE REGION ${v_mig_id} FROM ${v_mig_from_dn_id2} TO ${v_mig_dest_dn_id};"`
   echo "exp error:${v_exp_error}"
   sleep 2
...
function pre_and_exec_mig_region()
{
  ...
  v_mig_from_dn_id3=`awk "NR==3" ${cur_dir}/mig_id_info.txt|awk -F ',' '{print $1}'`
  ...
  v_mig_dest_dn_id1=`grep -v ${v_mig_from_dn_ip1} ${cur_dir}/all_dn_id_ip.txt|grep -v ${v_mig_from_dn_ip2}|grep -v ${v_mig_from_dn_ip3}|head -1 |awk -F ',' '{print $1}'`
  v_mig_dest_dn_id2=`grep -v ${v_mig_from_dn_ip1} ${cur_dir}/all_dn_id_ip.txt|grep -v ${v_mig_from_dn_ip2}|grep -v ${v_mig_from_dn_ip3}|tail -1 |awk -F ',' '{print $1}'`
