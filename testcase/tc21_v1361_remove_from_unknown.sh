...
#start 2bm
   ...
   nohup sh -x ${bm_dir}/benchmark.sh -cf ${bm_dir}/lt_10type_user_no_ssl/conf1 >${bm_dir}/${v_t}_tc21_bm1.out &
   nohup sh -x ${bm_dir}/benchmark.sh -cf ${bm_dir}/lt_10type_user_no_ssl/conf2 >${bm_dir}/${v_t}_tc21_bm2.out &
   sleep 60
# extend region
...
>${cur_dir}/region.txt
   ...
   echo "STOP DN ${v_rm_ip}: $(date "+%Y-%m-%d %H:%M:%S")"
...
#   wait_Removing_finish ${query_ip} 3600 ${v_rm_ip}
   sleep 2
   echo "START DN ${v_rm_ip}: $(date "+%Y-%m-%d %H:%M:%S")"
   start_dn ${v_rm_ip} ${query_ip}
   sleep 2
# repeate list
   ${cli_dir}/sbin/start-cli.sh -h ${query_ip} -e "REMOVE REGION ${v_remove_list},${v_remove_list}  FROM ${v_rm_id};">${cur_dir}/tmp.out
   check_res2 "successfully submitted:" "Target DataNode ${v_rm_id} is not in Running status" 1 "${SCRIPT_NAME}"
   cat ${cur_dir}/tmp.out
