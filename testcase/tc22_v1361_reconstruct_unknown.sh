...
#start 2bm
   ...
   nohup sh -x ${bm_dir}/benchmark.sh -cf ${bm_dir}/lt_10type_user_no_ssl/conf1 >${bm_dir}/${v_t}_tc22_bm1.out &
   nohup sh -x ${bm_dir}/benchmark.sh -cf ${bm_dir}/lt_10type_user_no_ssl/conf2 >${bm_dir}/${v_t}_tc22_bm2.out &
   sleep 60
# extend region
...
>${cur_dir}/region.txt
   ...
   check_res "Target DataNode ${v_rm_id} is not in Running status" 1 "${SCRIPT_NAME}"
   cat ${cur_dir}/tmp.out
...
# repeate reconstruc 
   ...
   wait_bm_finish 36000 "${bm_dir}/${v_t}_tc22_bm1.out" "${bm_dir}/${v_t}_tc22_bm2.out"
