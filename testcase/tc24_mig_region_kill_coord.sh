...
v_start_dn_ip=$1
 v_start_time=`date +%s`
  ssh ${u_name}@${v_start_dn_ip} "source /etc/profile;sudo ${db_dir}/sbin/start-datanode.sh -H ${db_dir}/dn_${v_start_time}_heapdump.hprof > /dev/null 2>&1 &"
  sleep 3
   while true
   do
      v_check_status=`${cli_dir}/sbin/start-cli.sh -h ${query_ip} -e "show datanodes"|grep ${v_start_dn_ip}|grep -i Running|wc -l`
...
function mig_region()
{
   ...
   while true
   do
      v_succ=`grep "Msg: The statement is executed successfully" ${cur_dir}/mig.out|wc -l`
      if [[ ${v_succ} = 1 ]];then
        break
      else
        v_fail=`grep "has some other region operation procedures in progress" ${cur_dir}/mig.out|wc -l`
        if [[ ${v_fail} -gt 0 ]];then
           sleep 30
           ${cli_dir}/sbin/start-cli.sh -h ${query_ip} -e "MIGRATE REGION ${v_mig_id} FROM ${v_mig_from_dn_id} TO ${v_mig_dest_dn_id};" > ${cur_dir}/mig.out
           let v_loop_rm++
           if [[ ${v_loop_rm} -gt 5 ]];then
              let fail_flag++
              return 0
           fi
        fi
...
function stop_coord_dn()
{
   ...
   do
       trans_num=0
       exec 7<${nodeinfo_dir}/datanode.txt
       while read line<&7
       do
              trans_num_tmp=`ssh ${u_name}@${line} "grep \"start to transmit snapshot\" ${db_dir}/logs/*datanode*all*|wc -l"`
              trans_num=$((trans_num+trans_num_tmp))
              if [[ ${trans_num} -ge ${mig_num} ]];then
                         v_dn_str=`ssh ${u_name}@${line} "sudo jps|grep DataNode|grep -v grep"`
                         v_dn_pid=`echo ${v_dn_str}|awk '{print $1}'`
                         ssh ${u_name}@${line} "sudo kill -9 ${v_dn_pid}"
                          while true
                          do
                              v_check_status=`ssh ${u_name}@${line} "sudo jps|grep -i datanode|wc -l"`
                              if [[ ${v_check_status} -gt 0 ]];then
                                 sleep 2
                              else
                                sleep 5
...
#check region status
                        t1=`date +%s`
                         while true
                         do
                            v_removing_num=`${cli_dir}/sbin/start-cli.sh -h ${query_ip} -timeout 36000 -e "show regions;"|grep Removing|wc -l`
                            v_adding_num=`${cli_dir}/sbin/start-cli.sh -h ${query_ip} -timeout 36000 -e "show regions;"|grep Adding|wc -l`
                            v_miging_num=$((v_removing_num+v_adding_num))
...
#                                break
   ...
   while true
   do
	   if [ -z "${v_coord_str}" ] && [[ ! ${v_coord_str} =~ "${v_mig_from_dn_ip1}" ]]; then
	      v_start_transmit_snapshot=`ssh ${u_name}@${v_mig_from_dn_ip1} "grep \"start to transmit snapshot\" ${db_dir}/logs/*datanode*all*|wc -l"`
	      if [[ ${v_start_transmit_snapshot} -gt 0 ]];then
                 v_dn_str=`ssh ${u_name}@${v_mig_from_dn_ip1} "sudo jps|grep DataNode|grep -v grep"`
                 v_dn_pid=`echo ${v_dn_str}|awk '{print $1}'`
                 ssh ${u_name}@${v_mig_from_dn_ip1} "sudo kill -9 ${v_dn_pid}"

#		 ssh ${u_name}@${v_mig_from_dn_ip1} "sudo jps|grep -i 'datanode' |grep -v grep |awk '{print $2}'|sudo xargs -r kill" 
		  while true
		  do
		      v_check_status=`ssh ${u_name}@${v_mig_from_dn_ip1} "sudo jps|grep -i datanode|wc -l"`
		      if [[ ${v_check_status} -gt 0 ]];then
			 sleep 2 
		      else
			break 
		  ...
		  v_coord_str="${v_coord_str},${v_mig_from_dn_ip1}"
		 break
	      fi
	   fi 
	   if [ -z "${v_coord_str}" ] || [[ ! ${v_coord_str} =~ "${v_mig_from_dn_ip2}" ]]; then
	      v_start_transmit_snapshot=`ssh ${u_name}@${v_mig_from_dn_ip2} "grep \"start to transmit snapshot\" ${db_dir}/logs/*datanode*all*|wc -l"`
	      if [[ ${v_start_transmit_snapshot} -gt 0 ]];then
                 v_dn_str=`ssh ${u_name}@${v_mig_from_dn_ip2} "sudo jps|grep DataNode|grep -v grep"`
                 v_dn_pid=`echo ${v_dn_str}|awk '{print $1}'`
                 ssh ${u_name}@${v_mig_from_dn_ip2} "sudo kill -9 ${v_dn_pid}"

#	          ssh ${u_name}@${v_mig_from_dn_ip2} "sudo jps|grep -i 'datanode' |grep -v grep |awk '{print $2}'|sudo xargs -r kill"	
		  while true
		  do
		      v_check_status=`ssh ${u_name}@${v_mig_from_dn_ip2} "sudo jps|grep -i datanode|wc -l"`
		      if [[ ${v_check_status} -gt 0 ]];then
			 sleep 2 
		      else
			break 
		  ...
		  v_coord_str="${v_coord_str},${v_mig_from_dn_ip2}"
		 break
	      fi
	  fi
          sleep 2
