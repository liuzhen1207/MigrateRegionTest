...
# kill -9 "start to transmit snapshot" dn
function stop_coord_dn()
{
   ...
   while true
   do
      exec 7<${nodeinfo_dir}/confignode.txt
      while read line<&7
      do
         ssh ${u_name}@${line} "sudo gunzip ${db_dir}/log-confignode-all*"
         v_success=`ssh ${u_name}@${line} "grep \"\[MigrateRegion\] success\" ${db_dir}/logs/*confignode*all.log|wc -l"`
         v_fail=`ssh ${u_name}@${line} "grep \[MigrateRegion\] ${db_dir}/logs/*confignode*all.log|grep \"RegionMigrateProcedure will not continue\"|wc -l"`
         if [[ ${v_success} -gt 0 ]]||[[ ${v_fail} -gt 0 ]];then
            echo "success log : ${v_success} , fail log : ${v_fail}"
            break
         fi
      done
         if [[ ${v_success} -gt 0 ]]||[[ ${v_fail} -gt 0 ]];then
   ...
   # restart killed dn
         v_start_time=`date +%s`
   ssh ${u_name}@${killed_dn_ip} "source /etc/profile;sudo ${db_dir}/sbin/start-datanode.sh -H ${db_dir}/dn_${v_start_time}_heapdump.hprof > /dev/null 2>&1 &"
   while true
   do
      v_check_status=`${cli_dir}/sbin/start-cli.sh -h ${query_ip} -e "show datanodes"|grep ${killed_dn_ip}|grep -i Running|wc -l`
      if [[ ${v_check_status} -gt 0 ]];then
         break
      else
         v_end_time=`date +%s`
