...
if ! wait_sync_done 60 1800;then
   ...
   query_ip2=`tail -1 ${cur_dir}/tmp.out`
      ...
      # A restarted DataNode may need to catch up.  Do not stop the next
      # replica until every Running DataNode has reported syncLag=0 again.
      if ! wait_sync_done 60 1800;then
...
sed -i "s/^PASSWORD=.*/PASSWORD=${bm_root_pw}/g" ${bm_dir}/lt_10type_user_no_ssl/conf*/config.properties
   ...
   nohup sh -x ${bm_dir}/benchmark.sh -cf ${bm_dir}/lt_10type_user_no_ssl/conf1 >${bm_dir}/${v_t}_tc24_bm1.out &
   nohup sh -x ${bm_dir}/benchmark.sh -cf ${bm_dir}/lt_10type_user_no_ssl/conf2 >${bm_dir}/${v_t}_tc24_bm2.out &
   sleep 60
# extend region
...
#   wait_Removing_finish ${query_ip} 3600 ${v_rm_ip}
   ...
   wait_bm_finish 36000 "${bm_dir}/${v_t}_tc24_bm1.out" "${bm_dir}/${v_t}_tc24_bm2.out"
