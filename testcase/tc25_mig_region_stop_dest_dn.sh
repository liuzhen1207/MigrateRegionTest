...
function stop_dest_dn1()
{
   ...
   do
		 ...
		 ssh ${u_name}@${v_mig_dest_dn_ip1} "source /etc/profile;sudo ${db_dir}/sbin/stop-datanode.sh"
		  while true
		  do
...
function stop_dest_dn2()
{
   ...
   do
              ...
              if [[ ${v_start_transmit_snapshot} -gt 1 ]];then
                 ssh ${u_name}@${v_mig_dest_dn_ip2} "source /etc/profile;sudo ${db_dir}/sbin/stop-datanode.sh"
                  while true
                  do
...
function pre_and_exec_mig_region()
{
 ...
 if [[ ${fail_flag} = 0 ]];then
  ...
  mig_region "${v_mig_id}" "${v_mig_from_dn_id2}" "${v_mig_dest_dn_id2}" "The statement is executed successfully"
  stop_dest_dn2
 fi 
