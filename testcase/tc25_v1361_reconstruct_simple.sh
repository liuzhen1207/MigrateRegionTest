...
set_sys_conf ${line} ${db_dir} ".*schema_region_consensus_protocol_class=.*" "schema_region_consensus_protocol_class=org.apache.iotdb.consensus.simple.SimpleConsensus"
...
#start 2bm
   ...
   nohup sh -x ${bm_dir}/benchmark.sh -cf ${bm_dir}/lt_10type_user_no_ssl/conf1 >${bm_dir}/${v_t}_tc25_bm1.out &
   nohup sh -x ${bm_dir}/benchmark.sh -cf ${bm_dir}/lt_10type_user_no_ssl/conf2 >${bm_dir}/${v_t}_tc25_bm2.out &
   sleep 60
# extend region
...
# repeate list
   ...
   ${cli_dir}/sbin/start-cli.sh -h ${query_ip} -e "reconstruct REGION ${v_remove_list}  on ${v_rm_id};">${cur_dir}/tmp.out 
   check_res "only has 1 replica, it cannot be reconstructed" 1 "${SCRIPT_NAME}" 
   cat ${cur_dir}/tmp.out
#   wait_Removing_finish ${query_ip} 3600 ${v_rm_ip}
   sleep 2
   wait_Adding_finish ${query_ip} 3600
   wait_Removing_finish ${query_ip} 3600
   wait_bm_finish 36000 "${bm_dir}/${v_t}_tc25_bm1.out" "${bm_dir}/${v_t}_tc25_bm2.out"
...
tc_res=true
  ...
  if [[ ${fail_flag} = 0 ]];then
     ...
     rm -rf ${bm_dir}/${v_t}_tc25_bm2.out 
     rm -rf ${bm_dir}/${v_t}_tc25_bm1.out 
  else
     tc_res=false
