...
function stop_dest_dn1()
{
   ...
   if ssh ${u_name}@${v_mig_from_dn_ip3} "[ -f ${db_dir}/logs/log-datanode-all*gz ]"; then
      ssh ${u_name}@${v_mig_from_dn_ip3} "sudo gunzip ${db_dir}/logs/log-datanode-all*"
   fi
   t1=`date +%s`
   while true
   do
              ...
              v_start_transmit_snapshot3=`ssh ${u_name}@${v_mig_from_dn_ip3} "grep \"start to transmit snapshot\" ${db_dir}/logs/*datanode*all*|wc -l"`
...
#                 ssh ${u_name}@${v_mig_dest_dn_ip1} "sudo kill -9 ${v_dn_pid}"
                 ssh ${u_name}@${v_mig_dest_dn_ip1} "sudo ${db_dir}/sbin/stop-datanode.sh"
...
#                 ssh ${u_name}@${v_mig_dest_dn_ip2} "sudo kill -9 ${v_dn_pid}"
                 ssh ${u_name}@${v_mig_dest_dn_ip2} "sudo ${db_dir}/sbin/stop-datanode.sh"
...
function pre_and_exec_mig_region()
{
  ...
  v_mig_from_dn_ip3=`awk "NR==3" ${cur_dir}/mig_id_info.txt|awk -F ',' '{print $2}'`
...
do
   ...
   v_error=`ssh ${u_name}@${line} "grep \"Meet error when estimate inner compaction memory\" ${db_dir}/logs/*datanode*all*|wc -l"`
   if [[ ${v_error} -gt 0 ]];then
      echo "${line},Meet error when estimate inner compaction memory,${v_error} rows."
      let fail_flag++
   fi
done
#exec 3<${nodeinfo_dir}/datanode.txt
...
#do
#
#   v_error=`ssh ${u_name}@${line} "grep \"IllegalArgumentException: Invalid input\" ${db_dir}/logs/*datanode*all*|wc -l"`
#   if [[ ${v_error} -gt 0 ]];then
#      echo "${line},IllegalArgumentException: Invalid input,${v_error} rows."
#      let fail_flag++
#   fi
#
#done
