...
#  ${cli_dir}/sbin/start-cli.sh -h ${query_ip} -timeout 36000 -e "select count(s_12),count(s_23),count(s_8),count(s_40),count(s_36),count(s_9),max_time(s_17),max_time(s_29),max_time(s_8),max_time(s_49),max_time(s_36),max_time(s_9) from root.** align by device;">${cur_dir}/q_act.out
 ...
 v_start_time=`date +%s`
  if [[ ${v_submit_fail1} = 1 ]];then
  ssh ${u_name}@${v_mig_dest_dn_ip1} "source /etc/profile;sudo ${db_dir}/sbin/start-datanode.sh -H ${db_dir}/dn_restart_${v_start_time}_heapdump.hprof > /dev/null 2>&1 &"
  fi
  sleep 1
  if [[ ${v_submit_fail2} = 1 ]];then
  ssh ${u_name}@${v_mig_dest_dn_ip2} "source /etc/profile;sudo ${db_dir}/sbin/start-datanode.sh -H ${db_dir}/dn_restart_${v_start_time}_heapdump.hprof > /dev/null 2>&1 &"
  fi
  sleep 1
   while true
   do
