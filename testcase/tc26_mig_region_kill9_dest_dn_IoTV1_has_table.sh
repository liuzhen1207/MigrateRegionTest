...
function mig_region()
{
   ...
   v_check=`grep "successful" ${cur_dir}/mig.out|wc -l`
...
function stop_dest_dn()
{
   stop_dest_dn_ip=$1
   loop_adding_num=0
   if [[ ${mig_flag} = "success" ]];then
	   while true
	   do
		      adding_num=`${cli_dir}/sbin/start-cli.sh -h ${query_ip} -timeout 600 -e "show regions"|grep Adding|wc -l`
		      if [[ ${adding_num} -gt 0 ]];then
                         let loop_adding_num++
                         if [[ ${loop_adding_num} -gt 3 ]];then
				 ssh ${u_name}@${stop_dest_dn_ip} "source /etc/profile;sudo ${db_dir}/sbin/stop-datanode.sh"
				  while true
				  do
				      v_check_status=`ssh ${u_name}@${stop_dest_dn_ip} "sudo jps|grep -i datanode|wc -l"`
				      if [[ ${v_check_status} -gt 0 ]];then
					 sleep 2
				      else
                                        echo "${stop_dest_dn_ip}" >>${cur_dir}/stop_ip_list.txt
					break
				      fi
				  done
				 break
   ...
   if [[ ${mig_flag} = "success" ]];then
           ...
           do
                      ...
                      if [[ ${adding_num} -gt 0 ]];then
                         let loop_adding_num++
                         if [[ ${loop_adding_num} -gt 3 ]];then
                                 v_dest_pid_str=`ssh ${u_name}@${stop_dest_dn_ip} "source /etc/profile;sudo jps|grep -i datanode"`
                                 v_dest_pid=`echo ${v_dest_pid_str}|awk '{print $1}'`
                                 if [[ ${v_dest_pid} -gt 0 ]];then
					 ssh ${u_name}@${stop_dest_dn_ip} "source /etc/profile;sudo kill -9 ${v_dest_pid}"
                                         sleep 1
					  while true
					  do
					      v_check_status=`ssh ${u_name}@${stop_dest_dn_ip} "sudo jps|grep -i datanode|wc -l"`
...
function check_mig_is_finish()
{
   # check Adding Removing
   loop_sum_no_mig_num=0
           while true
           do
                      adding_num=`${cli_dir}/sbin/start-cli.sh -h ${query_ip} -timeout 600 -e "show regions"|grep Adding|wc -l`
                      removing_num=`${cli_dir}/sbin/start-cli.sh -h ${query_ip} -timeout 600 -e "show regions"|grep Removing|wc -l`
                      sum_mig_num=$((adding_num+removing_num))
                      if [[ ${sum_mig_num} = 0 ]];then
                         let loop_sum_no_mig_num++
                         if [[ ${loop_sum_no_mig_num} -gt 5 ]];then
                             break
                         fi
                      else
                         sleep 1
...
function start_dn()
{
   start_ip=$1
   v_start_time=`date +%s`
   ssh ${u_name}@${start_ip} "source /etc/profile;sudo ${db_dir}/sbin/start-datanode.sh -H ${db_dir}/dn_${v_start_time}_heapdump.hprof > /dev/null 2>&1 &"
   while true
   do
      v_check_status=`${cli_dir}/sbin/start-cli.sh -h ${query_ip} -e "show datanodes"|grep ${start_ip}|grep -i Running|wc -l`
      if [[ ${v_check_status} -gt 0 ]];then
         break
      else
         v_end_time=`date +%s`
...
>${cur_dir}/stop_ip_list.txt
>${cur_dir}/ignore_dn_list.txt
  mig_region "${v_mig_id}" "${v_mig_from_dn_id1}" "${v_mig_dest_dn_id1}" "The statement is executed successfully"
  kill9_dest_dn "${v_mig_dest_dn_ip1}" 
  mig_region "${v_mig_id}" "${v_mig_from_dn_id2}" "${v_mig_dest_dn_id2}" "Submit RegionMigrateProcedure failed"
  kill9_dest_dn "${v_mig_dest_dn_ip2}"
  check_mig_is_finish
  exec 3<${cur_dir}/stop_ip_list.txt
  while read line<&3
  do
    start_dn ${line}
  done 
