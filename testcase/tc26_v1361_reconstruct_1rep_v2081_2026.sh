...
set_sys_conf ${line} ${db_dir} ".*schema_region_consensus_protocol_class=.*" "schema_region_consensus_protocol_class=org.apache.iotdb.consensus.ratis.RatisConsensus"
...
sed -i "s/^PASSWORD=.*/PASSWORD=${bm_root_pw}/g" ${bm_dir}/lt_10type_user_no_ssl/conf*/config.properties
   ...
   nohup sh -x ${bm_dir}/benchmark.sh -cf ${bm_dir}/lt_10type_user_no_ssl/conf1 >${bm_dir}/${v_t}_tc26_bm1.out &
   nohup sh -x ${bm_dir}/benchmark.sh -cf ${bm_dir}/lt_10type_user_no_ssl/conf2 >${bm_dir}/${v_t}_tc26_bm2.out &
   sleep 60
# extend region
...
#   wait_Removing_finish ${query_ip} 3600 ${v_rm_ip}
   ...
   wait_bm_finish 36000 "${bm_dir}/${v_t}_tc26_bm1.out" "${bm_dir}/${v_t}_tc26_bm2.out"
...
tc_res=true
  ...
  if [[ ${fail_flag} = 0 ]];then
     tc_res=true
     rm -rf ${bm_dir}/${v_t}_tc26_bm2.out
     rm -rf ${bm_dir}/${v_t}_tc26_bm1.out
