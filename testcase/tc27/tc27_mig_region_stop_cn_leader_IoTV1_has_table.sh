...
function stop_dest_cn()
{
   ...
   do
              ...
              v_start_transmit_snapshot22=`ssh ${u_name}@${v_mig_from_dn_ip3} "grep \"start to transmit snapshot\" ${db_dir}/logs/*datanode*all*|wc -l"`
              if [[ ${v_snapshot_num} -gt 0 ]];then
                 v_start_transmit_snapshot3=`ssh ${u_name}@${v_mig_dest_dn_ip1} "grep \"start to transmit snapshot\" ${db_dir}/logs/*datanode*all*|wc -l"`
                 v_start_transmit_snapshot=$((v_start_transmit_snapshot1+v_start_transmit_snapshot2+v_start_transmit_snapshot3+v_start_transmit_snapshot22))
              else
                 v_start_transmit_snapshot=$((v_start_transmit_snapshot1+v_start_transmit_snapshot2+v_start_transmit_snapshot22))
              fi
