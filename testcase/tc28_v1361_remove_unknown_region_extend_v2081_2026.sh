...
function start_dn()
{
   ...
   local v_query_ip=$2 
   # start rm_dn_ip
   v_t1=`date "+%Y_%m_%d_%H_%M_%S"`
   ssh ${u_name}@${rm_dn_ip} "source /etc/profile;sudo ${db_dir}/sbin/start-datanode.sh -H ${db_dir}/${v_t1}_restart_dn.hprof > /dev/null 2>&1 &"
   while true
   do
           sleep 5
...
sed -i "s/^PASSWORD=.*/PASSWORD=${bm_root_pw}/g" ${bm_dir}/lt_10type_user_no_ssl/conf*/config.properties
   ...
   sed -i "s/LOOP=.*/LOOP=7000/g" ${bm_dir}/lt_10type_user_no_ssl/conf*/config.properties
   nohup sh -x ${bm_dir}/benchmark.sh -cf ${bm_dir}/lt_10type_user_no_ssl/conf1 >${bm_dir}/${v_t}_tc28_bm1.out &
   nohup sh -x ${bm_dir}/benchmark.sh -cf ${bm_dir}/lt_10type_user_no_ssl/conf2 >${bm_dir}/${v_t}_tc28_bm2.out &
   sleep 60
# extend region
   ...
   ${cli_dir}/sbin/start-cli.sh -h ${query_ip} -u ${db_sys_admin} ${ssl_str} -e  'show data regions'|grep -v system|grep "${query_ip2}|"|awk -F '|' '{gsub(" ","");print $2}'>${cur_dir}/del_data_region_id.txt
   ${cli_dir}/sbin/start-cli.sh -h ${query_ip} -u ${db_sys_admin} ${ssl_str} -e  'show datanodes;'|grep Running|awk -F '|' '{gsub(" ","");print $2","}'>${cur_dir}/all_dn_id.txt
   v_del_region_num=`cat ${cur_dir}/del_data_region_id.txt|wc -l`
if [[ ${v_del_region_num} = 0 ]];then
let fail_flag++
test_end_sec=`date +%s`
...
v_del_region_id=`head -1 ${cur_dir}/del_data_region_id.txt`
   ${cli_dir}/sbin/start-cli.sh -h ${query_ip} -u ${db_sys_admin} ${ssl_str} -e  'show data regions'|grep "^| *${v_del_region_id}|"|grep -v "${query_ip2}|"|awk -F '|' '{gsub(" ","");print $8}'>${cur_dir}/del_data_region_another_dn_id.txt
fi
# delete tsfile
echo "delete tsfile TIME: $(date "+%Y-%m-%d %H:%M:%S")"
ssh ${u_name}@${v_rm_ip} "sudo find '${db_dir}/data/datanode/data/sequence/root.test.g_0/${v_del_region_id}' -name '*tsfile*' -delete"
stop_dn ${v_rm_ip} ${query_ip}
start_dn ${v_rm_ip} ${query_ip}
# remove this region
${cli_dir}/sbin/start-cli.sh -h ${query_ip} -e "REMOVE REGION ${v_del_region_id}  FROM ${v_rm_id};">${cur_dir}/tmp.out
check_res "successfully" 1 "${SCRIPT_NAME}"
sleep 5
wait_Removing_finish ${query_ip} 3600
v_from_dn_id=`head -1 ${cur_dir}/del_data_region_another_dn_id.txt`
${cli_dir}/sbin/start-cli.sh -h ${query_ip} -e "EXTEND REGION  ${v_del_region_id}  TO ${v_rm_id};">${cur_dir}/tmp.out
check_res "successfully" 1 "${SCRIPT_NAME}"
sleep 5
wait_Adding_finish ${query_ip} 3600

   wait_bm_finish 36000 "${bm_dir}/${v_t}_tc28_bm1.out" "${bm_dir}/${v_t}_tc28_bm2.out"
