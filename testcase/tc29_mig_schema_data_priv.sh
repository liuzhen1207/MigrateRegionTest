...
function pre_and_exec_mig_region()
{
  ...
  # create user
  ${cli_dir}/sbin/start-cli.sh -h ${query_ip} -e  "create user liuzhen 'TimechoDB@2021';"
...
do
   ...
   v_exp_error=`${cli_dir}/sbin/start-cli.sh -h ${query_ip} -u liuzhen -pw TimechoDB@2021 -e "MIGRATE REGION ${v_mig_id} FROM ${v_mig_from_dn_id} TO ${v_mig_to_dn_id};"`
   echo ${v_exp_error}
   if [[ $v_exp_error != *"permissions for this operation"* ]]; then
      let fail_flag++
   fi
   break 
done
...
do
   ...
   v_exp_error=`${cli_dir}/sbin/start-cli.sh -h ${query_ip} -u liuzhen -pw TimechoDB@2021 -e "MIGRATE REGION ${v_mig_sr_id} FROM ${v_mig_from_dn_id} TO ${v_mig_to_dn_id};"`
   echo ${v_exp_error}
   if [[ $v_exp_error != *"permissions for this operation"* ]]; then
      let fail_flag++
   fi
