...
{
SCHEMA_REGION_EXPECT=1
DATA_REGION_EXPECT=1
# 创建一个临时文件，用于存储中间结果
TEMP_FILE=$(mktemp)
...
#start 2bm
   ...
   sed -i "s/LOOP=.*/LOOP=3000/g" ${bm_dir}/lt_10type_user_no_ssl/conf*/config.properties
   nohup sh -x ${bm_dir}/benchmark.sh -cf ${bm_dir}/lt_10type_user_no_ssl/conf1 >${bm_dir}/${v_t}_tc29_bm1.out &
   nohup sh -x ${bm_dir}/benchmark.sh -cf ${bm_dir}/lt_10type_user_no_ssl/conf2 >${bm_dir}/${v_t}_tc29_bm2.out &
   sleep 60
# extend region
   ${cli_dir}/sbin/start-cli.sh -h ${query_ip} -u ${db_sys_admin} ${ssl_str} -e  'show schema regions'|grep Running|awk -F '|' '{gsub(" ","");print $2","$8}'>${cur_dir}/mig_sr_list.txt
   ${cli_dir}/sbin/start-cli.sh -h ${query_ip} -u ${db_sys_admin} ${ssl_str} -e  'show data regions'|grep Running|awk -F '|' '{gsub(" ","");print $2","$8}'>${cur_dir}/mig_dr_list.txt
# remove region
v_num=0
exec 3<${cur_dir}/mig_sr_list.txt
while read line<&3
do
v_region_id=`echo ${line}|awk -F ',' '{print $1}'`
v_dn_id=`echo ${line}|awk -F ',' '{print $2}'`
${cli_dir}/sbin/start-cli.sh -h ${query_ip} -e "REMOVE REGION ${v_region_id}  FROM ${v_dn_id};">${cur_dir}/tmp.out
let v_num++
if [[ ${v_num} -lt 3 ]];then
check_res "successfully" 1 "${SCRIPT_NAME}"
else
check_res "only has 1 replica, it cannot be removed" 1 "${SCRIPT_NAME}"
v_num=0
fi
wait_Removing_finish ${query_ip} 3600
done
v_num=0
exec 3<${cur_dir}/mig_dr_list.txt
while read line<&3
do
v_region_id=`echo ${line}|awk -F ',' '{print $1}'`
v_dn_id=`echo ${line}|awk -F ',' '{print $2}'`
...
wait_Removing_finish ${query_ip} 3600
   ...
   wait_bm_finish 36000 "${bm_dir}/${v_t}_tc29_bm1.out" "${bm_dir}/${v_t}_tc29_bm2.out"
...
tc_res=true
  ...
  if [[ ${fail_flag} = 0 ]];then
     ...
     rm -rf ${bm_dir}/${v_t}_tc29_bm1.out 
     rm -rf ${bm_dir}/${v_t}_tc29_bm2.out 
  else
     tc_res=false
