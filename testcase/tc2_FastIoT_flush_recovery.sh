...
function test1()
{
  ...
  ${cli_dir}/sbin/start-cli.sh -h ${query_ip} -e "insert into root.db.d1(time,s1,s2,s3,s4,s5,s6,s7,s8,s9,s10) values(1,true,1.1,'hello1',1,1,1.9,1997-01-01T08:00:00.001+08:00,'Hong Kong',X'486f6e67204b6f6e6720426c6f6221','1997-07-01');"
  ${cli_dir}/sbin/start-cli.sh -h ${query_ip} -e "insert into root.db.d2(time,s1,s2,s3,s4,s5,s6,s7,s8,s9,s10) values(2,true,1.1,'hello1',1,1,1.9,1997-01-01T08:00:00.001+08:00,'Hong Kong',X'486f6e67204b6f6e6720426c6f6221','1997-07-01');"
  ${cli_dir}/sbin/start-cli.sh -h ${query_ip} -e "flush;"
  ${cli_dir}/sbin/start-cli.sh -h ${query_ip} -e "select s1,s2,s3,s4,s5,s6,s7,s8,s9,s10 from root.db.* align by device;"|grep root >q_exp.out
  #kill -9 cluster
  exec 3<${nodeinfo_dir}/datanode.txt
  while read node<&3
  do
...
for i in {1..60}
do
   ${cli_dir}/sbin/start-cli.sh -h ${query_ip} -e "select s1,s2,s3,s4,s5,s6,s7,s8,s9,s10 from root.db.* align by device;" |grep root > q_act.out
