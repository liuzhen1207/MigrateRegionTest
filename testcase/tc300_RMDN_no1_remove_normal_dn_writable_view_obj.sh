...
resolve_object_columns() {
  ...
  query_table_base_object_sql="select time,device_id,read_object(${table_base_object_col}) from ${table_db_name}.${table_base_name} order by time,device_id limit 20;"
  query_table_view_object_sql="select time,device_id,read_object(${table_view_object_col}) from ${table_db_name}.${table_view_name} order by time,device_id limit 20;"
  return 0
}
...
normalize_query_result() {
  ...
  awk '
    ...
    /\|/ {
      ...
      if (out ~ /^(Time|time|device_id|Device|Database|TableName)\|/) {
        next
      }
      if (out != "|") {
        print out
