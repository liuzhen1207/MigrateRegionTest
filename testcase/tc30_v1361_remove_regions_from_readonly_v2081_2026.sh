...
sed -i "s/^PASSWORD=.*/PASSWORD=${bm_root_pw}/g" ${bm_dir}/lt_10type_user_no_ssl/conf*/config.properties
   ...
   nohup sh -x ${bm_dir}/benchmark.sh -cf ${bm_dir}/lt_10type_user_no_ssl/conf1 >${bm_dir}/${v_t}_tc30_bm1.out &
   nohup sh -x ${bm_dir}/benchmark.sh -cf ${bm_dir}/lt_10type_user_no_ssl/conf2 >${bm_dir}/${v_t}_tc30_bm2.out &
   sleep 60
# extend region
...
# extend region to readonly
   ...
   check_res2 "successfully submitted: 0, failed to submit:" "Target DataNode ${v_rm_id} is not in Running status" 1 "${SCRIPT_NAME}" 
   cat ${cur_dir}/tmp.out
   ${cli_dir}/sbin/start-cli.sh -h ${v_rm_ip} -e "set system to running on local;">${cur_dir}/tmp.out
   check_res "successfully" 1 "${SCRIPT_NAME}"
   sleep 1
   ${cli_dir}/sbin/start-cli.sh -h ${v_rm_ip} -e "show cluster;" 
   echo "EXTEND REGION TIME: $(date "+%Y-%m-%d %H:%M:%S")"
   ${cli_dir}/sbin/start-cli.sh -h ${query_ip} -e "EXTEND REGION ${v_remove_list}  TO ${v_rm_id};">${cur_dir}/tmp.out
   check_res "Msg: The statement is executed successfully" 1 "${SCRIPT_NAME}"
   cat ${cur_dir}/tmp.out
   wait_Adding_finish ${query_ip} 3600
   wait_bm_finish 36000 "${bm_dir}/${v_t}_tc30_bm1.out" "${bm_dir}/${v_t}_tc30_bm2.out"
...
tc_res=true
  ...
  if [[ ${fail_flag} = 0 ]];then
     ...
     rm -rf ${bm_dir}/${v_t}_tc30_bm1.out 
     rm -rf ${bm_dir}/${v_t}_tc30_bm2.out 
  else
     tc_res=false
