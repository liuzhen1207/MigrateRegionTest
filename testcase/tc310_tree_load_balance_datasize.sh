...
table_base_workload="conf_tc310_tree1"
table_view_workload="conf_tc310_tree2"
all_workloads="${table_base_workload} ${table_view_workload}"

table_db_name="root.test.g_0"
table_base_name=""
table_view_name=""
table_base_object_col=""
table_view_object_col=""
query_table_base_sql="select count(s_0),count(s_1),count(s_2),count(s_3),count(s_4),count(s_5),count(s_6),count(s_7),count(s_8),count(s_9),count(s_10),count(s_11) from root.test.** align by device;"
...
resolve_table_objects() {
  run_cli_sql "${query_ip}" tree "show devices root.test.**;" "${run_artifact_dir}/show_tree_devices.out" 3600
  if [[ $(grep -c 'root.test.' "${run_artifact_dir}/show_tree_devices.out") -le 0 ]]; then
    append_warn "show devices root.test.** did not find tree benchmark data"
    let fail_flag++
    return 1
  fi
  return 0
...
collect_regions_table() {
  ...
  check_cli_success "${out_file}" "${stage} tree show regions" || return 1
  return 0
}
...
show_migrations_is_empty_table() {
  ...
  run_cli_sql "${query_ip}" tree "show migrations;" "${out_file}" 3600
  trim_file_crlf "${out_file}"
  if grep -Eq "Exception|ERROR|Error" "${out_file}"; then
    append_warn "tree show migrations failed"
    let fail_flag++
    return 1
  fi
...
show_table_regions_has_active_migration() {
  ...
  trim_file_crlf "${out_file}"
  if grep -Eq "Exception|ERROR|Error" "${out_file}"; then
    append_warn "tree show regions failed"
    let fail_flag++
    return 1
  fi
...
run_table_maintenance_sql() {
  ...
  run_cli_sql "${query_ip}" tree "${sql}" "${out_file}" 3600
...
compare_cn_leader_disk_usage_std() {
  ...
  log "cn leader disk usage std did not improve after first load balance, continue to validate by tree region balance result: value1=${cn_leader_disk_usage_std_before_first_load_balance}, value2=${cn_leader_disk_usage_std_after_first_load_balance}"
  return 0
}
...
validate_load_balance_effect() {
  ...
  log "tree size diff before load balance=${before_pct} after load balance=${after_pct}"

  if ! awk -v before="${before_pct}" -v after="${after_pct}" 'BEGIN { exit (after < before) ? 0 : 1 }'; then
    append_warn "tree region size diff did not improve after load balance: before=${before_pct}, after=${after_pct}"
    let fail_flag++
  fi

  if awk -v after="${after_pct}" -v target="${target_balance_size_diff_pct}" 'BEGIN { exit (after > target) ? 0 : 1 }'; then
    append_warn "tree region size diff after load balance is still ${after_pct}%, expected <= ${target_balance_size_diff_pct}%"
    let fail_flag++
  fi
  ...
  if cmp -s "${before_norm}" "${after_norm}"; then
    append_warn "load balance finished but tree region topology did not change"
    let fail_flag++
  fi
...
capture_query_result() {
  ...
  run_cli_sql "${host}" tree "${sql}" "${raw_file}" 3600
  if query_output_has_error "${raw_file}"; then
    append_warn "tree query execute failed on ${host}"
    let fail_flag++
    return 1
  fi
...
check_final_consistency() {
  ...
  compare_query_result "${run_artifact_dir}/q_base_exp.norm" "${run_artifact_dir}/q_base_act.norm" "tree query result differs after load balance" || return 1
  return 0
}
