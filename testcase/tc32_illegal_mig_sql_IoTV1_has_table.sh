...
backup_dir_on_cn_dn_host=/data/iotdb/autotest_backup/3db_test_data_table_IoTV1
...
if [[ ${v_err_res} != *"Submit RegionMigrateProcedure failed"* ]];then
   let fail_flag++
fi
...
if [[ ${v_err_res} != *"no target DataNode"* ]];then
   let fail_flag++
fi
...
v_err_res=`${cli_dir}/sbin/start-cli.sh -h ${query_ip} -timeout 36000 -e "migrate region 10 from 7 to 7;"`
if [[ ${v_err_res} != *"because the target DataNode 7 already contains Region 10"* ]];then
   let fail_flag++
fi
...
v_err_res=`${cli_dir}/sbin/start-cli.sh -h ${query_ip} -timeout 36000 -e "migrate region 10 from 7 to 4;"`
if [[ ${v_err_res} != *"because the target DataNode 4 already contains Region 10"* ]];then
   let fail_flag++
fi
