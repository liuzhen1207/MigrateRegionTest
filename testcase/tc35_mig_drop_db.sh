...
do
   ...
   do
      ssh ${u_name}@${v_cn_leader_ip} "sudo gunzip ${db_dir}/logs/log-confignode-all*"
              v_mig_suc_log=`ssh ${u_name}@${v_cn_leader_ip} "grep \"RegionMigrateProcedure will not continue\" ${db_dir}/logs/*confignode*all.log|tail -1"`
              v_mig_suc_time=`echo ${v_mig_suc_log}|awk -F , '{print $1}'`
              v_mig_suc_sec=`date -d"${v_mig_suc_time}" +%s`
              v_mig_suc_log_2=`ssh ${u_name}@${v_cn_leader_ip} "grep \"get region group id fail\" ${db_dir}/logs/*confignode*all.log|tail -1"`
              v_mig_suc_time_2=`echo ${v_mig_suc_log_2}|awk -F , '{print $1}'`
              v_mig_suc_sec_2=`date -d"${v_mig_suc_time_2}" +%s`
              if [[ ${v_mig_suc_sec} -gt ${v_bef_mig_sec} ]];then
                 break
              elif [[ ${v_mig_suc_sec_2} -gt ${v_bef_mig_sec} ]];then
                 break
              else
                 sleep 2 
              fi
              v_Adding_num=`${cli_dir}/sbin/start-cli.sh -h ${query_ip} -e "show regions;"|grep Adding|wc -l`
              v_Removing_num=`${cli_dir}/sbin/start-cli.sh -h ${query_ip} -e "show regions;"|grep Removing|wc -l`
