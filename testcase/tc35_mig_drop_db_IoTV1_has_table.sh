...
do
   ...
   let v_mig_num++
   if [[ ${v_mig_num} = 1 ]];then
      v_exp_submit_suc=`grep success ${cur_dir}/mig.out|wc -l`
      if [[ ${v_exp_submit_suc} = 0 ]];then
         let fail_flag++
      fi
   else
        v_exp_submit_fail=`grep "get region group id fail" ${cur_dir}/mig.out|wc -l`
        if [[ ${v_exp_submit_fail} = 0 ]];then
           let fail_flag++
        fi
   ...
   if [[ ${v_del_flag} -gt 0 ]];then
      ...
      ${cli_dir}/sbin/start-cli.sh -h ${query_ip} -e "drop database root.test;">${cur_dir}/del_s_40.out &
      ${cli_dir}/sbin/start-cli.sh -h ${query_ip} -e "drop database root.test.g_0;">>${cur_dir}/del_s_40.out &

      let v_del_flag++
   fi
   v_cn_leader_ip=`${cli_dir}/sbin/start-cli.sh -h ${query_ip} -e "show confignodes;"|grep Leader|awk -F '|' '{gsub(" ","");print $4}'`
   v_cn_ip1=`${cli_dir}/sbin/start-cli.sh -h ${query_ip} -e "show confignodes;"|grep -i running|grep -v Leader|head -1|awk -F '|' '{gsub(" ","");print $4}'`
   v_cn_ip2=`${cli_dir}/sbin/start-cli.sh -h ${query_ip} -e "show confignodes;"|grep -i running|grep -v Leader|tail -1|awk -F '|' '{gsub(" ","");print $4}'`
   t1=`date +%s`
   while true
   do
      ssh ${u_name}@${v_cn_leader_ip} "sudo gunzip ${db_dir}/logs/log-confignode-all*"
      ssh ${u_name}@${v_cn_ip1} "sudo gunzip ${db_dir}/logs/log-confignode-all*"
      ssh ${u_name}@${v_cn_ip2} "sudo gunzip ${db_dir}/logs/log-confignode-all*"
              v_mig_suc_log1=`ssh ${u_name}@${v_cn_leader_ip} "grep \"RegionMigrateProcedure will not continue\" ${db_dir}/logs/*confignode*all.log|wc -l"`
              v_mig_suc_log2=`ssh ${u_name}@${v_cn_ip1} "grep \"RegionMigrateProcedure will not continue\" ${db_dir}/logs/*confignode*all.log|wc -l"`
              v_mig_suc_log3=`ssh ${u_name}@${v_cn_ip2} "grep \"RegionMigrateProcedure will not continue\" ${db_dir}/logs/*confignode*all.log|wc -l"`
              v_mig_not_continue=$((v_mig_suc_log1+v_mig_suc_log2+v_mig_suc_log3))
              if [[ ${v_mig_not_continue} = 0 ]];then
                 sleep 5
              elif [[ ${v_mig_not_continue} = 1 ]];then
                 break
              else
                 t2=`date +%s`
                 t3=$((t2-t1))
...
if [[ ${v_check_mig_regionid} != 0 ]];then
   let fail_flag++
fi
