...
function remove_cn()
{
   ...
   v_rm_id=`cat ${cur_dir}/tmp.out |grep  Leader|grep Running|tail -1|awk -F "|" '{gsub(" ","");print $2}'`
   v_rm_ip=`cat ${cur_dir}/tmp.out |grep  Leader|grep Running|tail -1|awk -F "|" '{gsub(" ","");print $4}'`
