...
#start 2bm
   ...
   nohup sh -x ${bm_dir}/benchmark.sh -cf ${bm_dir}/lt_10type_user_no_ssl/conf1 >${bm_dir}/${v_t}_tc4_sys_admin_bm1.out &
   nohup sh -x ${bm_dir}/benchmark.sh -cf ${bm_dir}/lt_10type_user_no_ssl/conf2 >${bm_dir}/${v_t}_tc4_sys_admin_bm2.out &
   sleep 30
   ${cli_dir}/sbin/start-cli.sh -u ${db_sec_admin} ${ssl_str} -h ${query_ip} -e "create user lily 'TimechoDB@2021';">${cur_dir}/tmp.out
   check_res "success" 1 "${SCRIPT_NAME}"
   ${cli_dir}/sbin/start-cli.sh -u lily ${ssl_str} -h ${query_ip} -e "remove datanode ${v_rm_id};">${cur_dir}/tmp.out
   check_res "803: No permissions for this operation, please add privilege SYSTEM" 1 "${SCRIPT_NAME}"
  ${cli_dir}/sbin/start-cli.sh -u ${db_sec_admin} ${ssl_str} -h ${query_ip} -e "remove datanode ${v_rm_id};">${cur_dir}/tmp.out
   check_res "803: No permissions for this operation, please add privilege SYSTEM" 1 "${SCRIPT_NAME}"
  ${cli_dir}/sbin/start-cli.sh -u audit_admin ${ssl_str} -h ${query_ip} -e "remove datanode ${v_rm_id};">${cur_dir}/tmp.out
   check_res "803: No permissions for this operation, please add privilege SYSTEM" 1 "${SCRIPT_NAME}"

   ${cli_dir}/sbin/start-cli.sh -u ${db_sys_admin} ${ssl_str} -h ${query_ip} -e "grant system on root.** to user lily;">${cur_dir}/tmp.out
   check_res "success" 1 "${SCRIPT_NAME}"
   cat ${cur_dir}/tmp.out
   ${cli_dir}/sbin/start-cli.sh -u audit_admin ${ssl_str} -h ${query_ip} -e "select * from root.__audit.password_history.**;">${cur_dir}/tmp.out
   cat ${cur_dir}/tmp.out
   ${cli_dir}/sbin/start-cli.sh -u lily ${ssl_str} -h ${query_ip} -e "remove datanode ${v_rm_id};">${cur_dir}/tmp.out
   check_res "success" 1 "${SCRIPT_NAME}"
...
# stop removing dn
   sleep 5
   ssh ${u_name}@${v_rm_ip} "source /etc/profile;cd ${db_dir};sudo ./sbin/stop-datanode.sh"
# wait bm finish
   wait_bm_finish 3600 
   wait_rm_finish "${v_rm_ip}" 600 
