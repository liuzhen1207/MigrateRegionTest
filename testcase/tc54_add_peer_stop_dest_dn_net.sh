...
function set_conf()
{
  ...
  do
     ...
     ssh ${u_name}@${line} "sed -i 's/# default_schema_region_group_num_per_database=.*/default_schema_region_group_num_per_database=1/g' ${db_dir}/conf/iotdb-common.properties"
     ssh ${u_name}@${line} "sed -i 's/# default_data_region_group_num_per_database=.*/default_data_region_group_num_per_database=5/g' ${db_dir}/conf/iotdb-common.properties"
     ssh ${u_name}@${line} "sed -i 's/.*region_migration_speed_limit_bytes_per_second.*/region_migration_speed_limit_bytes_per_second=33554432/g' ${db_dir}/conf/iotdb-common.properties"
...
function restart_net()
{
   v_ip=$1
   v_sleep=$2
   echo "#!/bin/bash" > ${cur_dir}/restart_net.sh
   echo "sudo ifconfig eth0 down" >> ${cur_dir}/restart_net.sh
   echo "sleep ${v_sleep}" >> ${cur_dir}/restart_net.sh
   echo "sudo ifconfig eth0 up" >> ${cur_dir}/restart_net.sh
   scp -rp ${cur_dir}/restart_net.sh ${u_name}@${line}:~/
   ssh ${u_name}@${v_ip} "sh -x ~/restart_net.sh >/dev/null 2>&1 &"
}
...
# check snapshot
   ...
   do
      ...
      if [[ ${v_AddRegion} -gt 1 ]];then
         restart_net ${v_remove_coord_ip} 60
         break
      else
         sleep 1
      fi
...
sleep 1
   ...
   if [[ ${v_pid} -gt 0 ]];then
      echo "stop network failed,please check your testcase."
      let fail_flag++ 
   fi
...
#check reset peer list automatically
     ssh ${u_name}@${v_cn_leader_ip} "sudo gunzip ${db_dir}/logs/log-confignode-all*"
              v_mig_suc_log=`ssh ${u_name}@${v_cn_leader_ip} "grep \"reset peer list automatically\" ${db_dir}/logs/*confignode*all*|wc -l"`
              v_mig_suc_log_2=`ssh ${u_name}@${v_cn_leader_ip} "grep \"sub-procedure AddRegionPeerProcedure fail, RegionMigrateProcedure will not continue\" ${db_dir}/logs/*confignode*all*|wc -l"`
              if [[ ${v_mig_suc_log} != 2 ]];then
                 let fail_flag++
              fi
              if [[ ${v_mig_suc_log_2} != 2 ]];then
                 let fail_flag++
              fi
