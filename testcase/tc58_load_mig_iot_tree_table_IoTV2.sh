...
# start repair data
${cli_dir}/sbin/start-cli.sh -h ${query_ip}  -timeout 600 -e "start repair data;"
	exec 3<${nodeinfo_dir}/datanode.txt
	while read line <&3
	do
           while true
           do
		   ssh ${u_name}@${line} "sudo gunzip ${db_dir}/logs/log-datanode-all*" 
		   v_finish=`ssh ${u_name}@${line} "grep \"Repair task finished\" ${db_dir}/logs/*datanode*all*|wc -l"`
		   if [[ ${v_finish} -gt 0 ]];then
		      break
		   else
		      sleep 5
		   fi
...
# check logs need to repair
exec 3<${nodeinfo_dir}/datanode.txt
while read line <&3
do
   ssh ${u_name}@${line} "sudo gunzip ${db_dir}/logs/log-datanode-all*"
   v_npe=`ssh ${u_name}@${line} "grep \"need to repair\" ${db_dir}/logs/*datanode*all*|wc -l"`
   if [[ ${v_npe} -gt 0 ]];then
      let fail_flag++
      v_time=`date +%s`
      ssh ${u_name}@${line} "sudo cp -rp ${db_dir}/logs ${db_dir}/logs_dn_${v_time}_npe"
