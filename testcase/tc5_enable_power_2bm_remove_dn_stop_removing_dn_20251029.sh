...
#start 2bm
   ...
   nohup sh -x ${bm_dir}/benchmark.sh -cf ${bm_dir}/lt_10type_user_no_ssl/conf1 >${bm_dir}/${v_t}_tc5_sys_admin_bm1.out &
   nohup sh -x ${bm_dir}/benchmark.sh -cf ${bm_dir}/lt_10type_user_no_ssl/conf2 >${bm_dir}/${v_t}_tc5_sys_admin_bm2.out &
   sleep 30
...
# stop DR-Adding dn
  while true
  do 
   v_stop_removing_ip=`${cli_dir}/sbin/start-cli.sh -u ${db_sys_admin} ${ssl_str} -h ${query_ip} -e "show data regions;"|grep Removing|tail -1|awk -F "|" '{gsub(" ","");print $9}'`
   if [ -n "$v_stop_removing_ip" ]; then
   ssh ${u_name}@${v_stop_removing_ip} "source /etc/profile;cd ${db_dir};sudo ./sbin/stop-datanode.sh"
   echo "Stopping ${v_stop_removing_ip}"
   break
   fi
   sleep 1
  done
  if [[ ${query_ip} = ${v_stop_removing_ip} ]];then
     query_ip=`tail -1 ${nodeinfo_dir}/datanode.txt` 
  fi
  check_stop ${v_stop_removing_ip} ${query_ip}
   wait_rm_finish "${v_rm_ip}" 1200
#not restart
#   v_start_time=`date +%s`
#   ssh ${u_name}@${v_stop_removing_ip} "source /etc/profile;cd ${db_dir};sudo ./sbin/start-datanode.sh -H ${db_dir}/dn_${v_start_time}_heapdump.hprof > /dev/null 2>&1 &"
#   check_restart ${v_stop_removing_ip} ${query_ip} 
# wait bm finish
   wait_bm_finish 3600 
