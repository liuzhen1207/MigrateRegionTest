...
function test1()
{
  ...
  out_file="${test_time}_tc5.out"
  ssh ${u_name}@${bm_ip} "${bm_dir}/run_tc.sh ${bm_conn_ip} tc5.conf ${out_file} >/dev/null 2>&1"
  ...
  while true
  do
    ssh ${u_name}@${v_cn_leader_ip} "sudo gunzip log-confignode-all*"
...
# restart confignode took snapshot,stop leader cn
  while true
  do
    ssh ${u_name}@${stop_1_cn_ip} "sudo gunzip log-confignode-all*"
...
function check_res()
{
  ...
  do
     ...
     if [[ ${line} = ${query_ip} ]];then
        ...
        while true
        do
           v_query_finished=`jps|grep "select count"|wc -l`
           if [[ ${v_query_finished} -gt 0 ]];then
              v_end_sec=`date +%s`
              v_query_time=$((v_end_sec-v_begin_sec))
              if [[ ${v_query_time} -gt 60 ]];then
                 ${cli_dir}/sbin/start-cli.sh -h ${query_ip}  -timeout 36000 -e 'show query processlist;' &
                 sleep 10
                   exec 4<${nodeinfo_dir}/datanode.txt
		   while read dn_ip <&4
		   do
			   v_ip=`echo ${dn_ip} |awk -F '.' '{print $4}'`
			   v_pid_str=`ssh ${u_name}@${dn_ip} "sudo jps|grep -i datanode"`
			   v_pid=`echo ${v_pid_str} |awk '{print $1}'`
                           if [[ ${v_pid} -gt 0 ]];then
	  		      ssh ${u_name}@${dn_ip} "sudo jstack -l ${v_pid}" > stack_${v_end_sec}_${v_ip}.out
                           fi
		   done
                   let fail_flag++
     ...
     if [[ ${line} = ${query_ip} ]];then
        ...
        echo "q_res_1ts:${q_res_1ts},v_ts_num_per_dev:${v_ts_num_per_dev}"
