...
#start 2bm
   ...
   nohup sh -x ${bm_dir}/benchmark.sh -cf ${bm_dir}/lt_10type_user_no_ssl/conf1 >${bm_dir}/${v_t}_tc5_bm1.out &
   nohup sh -x ${bm_dir}/benchmark.sh -cf ${bm_dir}/lt_10type_user_no_ssl/conf2 >${bm_dir}/${v_t}_tc5_bm2.out &
   sleep 300
# stop dn ${v_rm_ip}
   if [ -n "$v_rm_ip" ]; then
   ssh ${u_name}@${v_rm_ip} "source /etc/profile;cd ${db_dir};sudo ./sbin/stop-datanode.sh"
   echo "Stopping ${v_rm_ip}"
   fi

  if [[ ${query_ip} = ${v_rm_ip} ]];then
     query_ip=`sed -n '2p' ${nodeinfo_dir}/datanode.txt`
  fi
  ...
  check_stop ${v_rm_ip} ${query_ip}
   ...
   wait_bm_finish 36000 "${bm_dir}/${v_t}_tc5_bm1.out" "${bm_dir}/${v_t}_tc5_bm2.out"
