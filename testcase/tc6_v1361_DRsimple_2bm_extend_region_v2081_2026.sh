...
set_sys_conf ${line} ${db_dir} ".*data_region_consensus_protocol_class=.*" "data_region_consensus_protocol_class=org.apache.iotdb.consensus.simple.SimpleConsensus"
  done
...
local t1=`date +%s`
   ...
   if grep -Eq "does not exist|Registering schema failed|BrokenBarrierException|NullPointerException" "${bm_res1}" "${bm_res2}";then
      let fail_flag++
...
#start 2bm
   ...
   nohup sh -x ${bm_dir}/benchmark.sh -cf ${bm_dir}/lt_10type_user_no_ssl/conf1 >${bm_dir}/${v_t}_tc6_bm1.out &
   nohup sh -x ${bm_dir}/benchmark.sh -cf ${bm_dir}/lt_10type_user_no_ssl/conf2 >${bm_dir}/${v_t}_tc6_bm2.out &
   sleep 30
   ${cli_dir}/sbin/start-cli.sh -u ${db_sys_admin} ${ssl_str} -h ${query_ip} -e "remove datanode ${v_rm_id};">${cur_dir}/tmp.out
   check_res "SimpleConsensus protocol is not supported to remove data node" 1 "${SCRIPT_NAME}"
# extend region
   ${cli_dir}/sbin/start-cli.sh -h ${query_ip} -u ${db_sys_admin} ${ssl_str} -e  'show data regions'|grep "${query_ip}|"|awk -F '|' '{gsub(" ","");print $2}'>${cur_dir}/mig_id.txt
   ${cli_dir}/sbin/start-cli.sh -h ${query_ip} -u ${db_sys_admin} ${ssl_str} -e  'show data regions'|grep "${query_ip2}|"|awk -F '|' '{gsub(" ","");print $2}'>${cur_dir}/mig_id2.txt
   exec 3<${cur_dir}/mig_id.txt
   while read line<&3
   do
   v_find_num=`grep -w ${line} ${cur_dir}/mig_id2.txt|wc -l`
   if [[ ${v_find_num} = 0 ]] &&  [[ -n "$line" ]];then
   echo "EXTEND DATA REGION TIME: $(date "+%Y-%m-%d %H:%M:%S")"
   ${cli_dir}/sbin/start-cli.sh -h ${query_ip} -e "EXTEND REGION ${line}  TO ${v_rm_id};">${cur_dir}/tmp.out
   check_res "SimpleConsensus not supports region operation" 1 "${SCRIPT_NAME}"
   fi
   done
   ${cli_dir}/sbin/start-cli.sh -h ${query_ip} -u ${db_sys_admin} ${ssl_str} -e  'show schema regions'|grep "${query_ip}|"|awk -F '|' '{gsub(" ","");print $2}'>${cur_dir}/mig_id.txt
   ${cli_dir}/sbin/start-cli.sh -h ${query_ip} -u ${db_sys_admin} ${ssl_str} -e  'show schema regions'|grep "${query_ip2}|"|awk -F '|' '{gsub(" ","");print $2}'>${cur_dir}/mig_id2.txt
   exec 3<${cur_dir}/mig_id.txt
   while read line<&3
   do
   v_find_num=`grep -w ${line} ${cur_dir}/mig_id2.txt|wc -l`
   if [[ ${v_find_num} = 0 ]] &&  [[ -n "$line" ]];then
   echo "EXTEND SCHEMA REGION TIME: $(date "+%Y-%m-%d %H:%M:%S")"
   ${cli_dir}/sbin/start-cli.sh -h ${query_ip} -e "EXTEND REGION ${line}  TO ${v_rm_id};">${cur_dir}/tmp.out
   check_res "successfully" 1 "${SCRIPT_NAME}"
   fi
   done

   wait_bm_finish 36000 "${bm_dir}/${v_t}_tc6_bm1.out" "${bm_dir}/${v_t}_tc6_bm2.out"
