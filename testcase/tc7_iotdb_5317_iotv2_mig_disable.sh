...
function mig_region()
{
  ...
  ${cli_dir}/sbin/start-cli.sh -h ${query_ip} -timeout 360000 -sql_dialect table -e "select device_id, count(s_0) from db_table_g_0.table_0 group by device_id order by device_id;;">${cur_dir}/q_exp_table.out
  ...
  v_stop_dn_ip=`cat ${cur_dir}/mig_id_info.txt|grep -v Leader|tail -1|awk -F ',' '{print $2}'`
  v_readonly_dn_ip=`cat ${cur_dir}/mig_id_info.txt|grep -v Leader|head -1|awk -F ',' '{print $2}'`
  v_mig_from_dn_id=`cat ${cur_dir}/mig_id_info.txt|grep -v Leader|head -1|awk -F ',' '{print $1}'`
# stop datanode
...
# set ReadOnly
   ${cli_dir}/sbin/start-cli.sh -h ${v_readonly_dn_ip} -e "set system to readonly on local"
   while true
   do
      if [[ ${query_ip} = ${v_stop_dn_ip} ]];then
         query_ip=`tail -1 ${nodeinfo_dir}/datanode.txt`
      fi
      v_readonly_res=`${cli_dir}/sbin/start-cli.sh -h ${query_ip} -e "show cluster"|grep -i datanode|grep ${v_readonly_dn_ip}|grep -i readonly|wc -l`
      if [[ ${v_readonly_res} -gt 0 ]];then
         break
      else
         sleep 1
      fi
...
# exec migrate
   ...
   ${cli_dir}/sbin/start-cli.sh -h ${query_ip} -e "show cluster" > ${cur_dir}/tc7_bef_mig_cluster.out
   ${cli_dir}/sbin/start-cli.sh -h ${query_ip} -e "show regions" >> ${cur_dir}/tc7_bef_mig_cluster.out
   ${cli_dir}/sbin/start-cli.sh -h ${query_ip} -e "show confignodes" >> ${cur_dir}/tc7_bef_mig_cluster.out
   ...
   while true
   do
      v_mig_suc=`ssh ${u_name}@${v_cn_leader_ip} "grep \"\[MigrateRegion\] success\" ${db_dir}/logs/log_confignode_all.log|wc -l"`
      if [[ ${v_mig_suc} -gt 0 ]];then
#              v_del_suc=`ssh ${u_name}@${v_cn_leader_ip} "grep \"DELETE_OLD_REGION_PEER failed\" ${db_dir}/logs/log_confignode_all.log|wc -l"`
#              if [[ ${v_del_suc} = 0 ]];then
#                 let fail_flag++
#              fi
  ...
  ${cli_dir}/sbin/start-cli.sh -h ${query_ip} -timeout 360000 -sql_dialect table -e "select device_id, count(s_0) from db_table_g_0.table_0 group by device_id order by device_id;;">${cur_dir}/q_act_table.out

  v_check_data=`grep root.test.g_0 ${cur_dir}/q_act.out|wc -l`
  if [[ ${v_check_data} = 0 ]];then
     let fail_flag++
  fi
   ...
   v_query_is_same=`diff ${cur_dir}/q_act_table.out ${cur_dir}/q_exp_table.out|grep d1_|wc -l`
   if [[ ${v_query_is_same} -gt 0 ]];then
      let fail_flag++
   fi

   v_dest_is_running=`${cli_dir}/sbin/start-cli.sh -h ${query_ip} -e "show regions"|grep " ${v_mig_id}|[[:space:]]*DataRegion"|grep Running|awk -F '|' '{gsub(" ","");print $8}'|grep ${v_mig_to_dn_id}|wc -l`
   if [[ ${v_dest_is_running} = 0 ]];then
      let fail_flag++
   fi
#restart unknown dn
v_start_time=`date +%s`
