...
backup_dir_on_cn_dn_host=/data/iotdb/autotest_backup/tc7_test_data_IoTV2
...
# exec migrate
   ...
   do
      ...
      v_mig_fail=`ssh ${u_name}@${v_cn_leader_ip} "grep \"\[MigrateRegion\] \" ${db_dir}/logs/log_confignode_all.log|grep \"RegionMigrateProcedure will not continue\"|wc -l"`
      if [[ ${v_mig_fail} -gt 0 ]];then
         let fail_flag++
         break
      fi
...
#              fi
      ...
      if [[ ${t3} -gt 1500 ]];then
         let fail_flag++
         echo "1500 s ,MigrateRegion not success."
         break
      fi
   done
