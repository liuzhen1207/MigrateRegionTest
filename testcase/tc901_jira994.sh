...
# 【TIMECHODB-994】 【ERROR日志处理】DataNode上没有SchemaRegion，重启ERROR：Failed to get all consensus group ids from disk 
exec 3<${nodeinfo_dir}/datanode.txt
while read line<&3
do
   ssh ${u_name}@${line} "sudo gunzip ${db_dir}/logs/log-datanode-all*"
   v_exp_log=`ssh ${u_name}@${line} "grep NoSuchFileException ${db_dir}/logs/*datanode*all*"|wc -l`
