...
function mig_region()
{
  ...
  v_mig_from_dn_id=`cat ${cur_dir}/mig_id_info.txt|grep -v Leader|tail -1|awk -F ',' '{print $1}'`
exec 3<${cur_dir}/mig_id_info.txt
while read line<&3
do
  v_stop_dn_ip=`echo ${line}|awk -F ',' '{print $2}'`
# stop datanode
...
# exec migrate
   ...
   while true
   do
      v_check_stop=`${cli_dir}/sbin/start-cli.sh -h ${query_ip} -e "show data regions"|grep " ${v_mig_id}|[[:space:]]*DataRegion"|grep -i unknown|wc -l`
      if [[ ${v_check_stop} = 3 ]];then
         break
      else
         sleep 2
      fi
...
#   while true 
#   do
#      v_mig_suc=`ssh ${u_name}@${v_cn_leader_ip} "grep \"RegionMigrateProcedure success,\" ${db_dir}/logs/log_confignode_all.log|wc -l"`
...
# check datanode pid
   while true
   do
	   v_stop_dn_pid=`ssh ${u_name}@${v_stop_dn_ip} "sudo jps|grep -i datanode|wc -l"`
	   if [[ ${v_stop_dn_pid} -gt 0 ]];then
	      sleep 1 
	   else
              v_stop_t2=`date +%s`
              v_stop_elp=$((v_stop_t2-v_stop_t1))
              echo "stop ${v_stop_dn_ip} cost:${v_stop_elp} sec."
	      break
	   fi
   done
done
...
ssh ${u_name}@${v_stop_dn_ip} "source /etc/profile;sudo ${db_dir}/sbin/start-datanode.sh -H ${db_dir}/dn_${v_start_time}_heapdump.hprof > /dev/null 2>&1 &" 
   ...
   while true
   do
      v_check_stop=`${cli_dir}/sbin/start-cli.sh -h ${query_ip} -e "show data regions"|grep " ${v_mig_id}|[[:space:]]*DataRegion"|grep -i Running|wc -l`
      if [[ ${v_check_stop} = 3 ]];then
         break
      else
         v_end_time=`date +%s`
...
# 172.20.70.27
   ssh ${u_name}@172.20.70.27 "sudo gunzip ${db_dir}/logs/log-datanode-all*"
   v_exp_log=`ssh ${u_name}@172.20.70.27 "grep NoSuchFileException ${db_dir}/logs/*datanode*all*"|wc -l`
   if [[ ${v_exp_log} -gt 0 ]];then
      let fail_flag++
   fi
