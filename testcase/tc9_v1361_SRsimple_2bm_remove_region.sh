...
#start 2bm
   ...
   nohup sh -x ${bm_dir}/benchmark.sh -cf ${bm_dir}/lt_10type_user_no_ssl/conf1 >${bm_dir}/${v_t}_tc9_bm1.out &
   nohup sh -x ${bm_dir}/benchmark.sh -cf ${bm_dir}/lt_10type_user_no_ssl/conf2 >${bm_dir}/${v_t}_tc9_bm2.out &
   sleep 30
...
# extend region
   ...
   if [[ -n "$v_remove_list" ]];then
   echo "REMOVE DATA REGION TIME: $(date "+%Y-%m-%d %H:%M:%S")"
   ...
   v_remove_list=`paste -sd "," ${cur_dir}/mig_id2.txt`
   if [[ -n "$v_remove_list" ]];then
   echo "REMOVE SCHEMA REGION TIME: $(date "+%Y-%m-%d %H:%M:%S")"
   ${cli_dir}/sbin/start-cli.sh -h ${query_ip} -e "REMOVE REGION ${v_remove_list}  FROM ${v_rm_id};">${cur_dir}/tmp.out
   check_res "successfully submitted: 0, failed to submit:" 1 "${SCRIPT_NAME}"
   fi

   wait_bm_finish 36000 "${bm_dir}/${v_t}_tc9_bm1.out" "${bm_dir}/${v_t}_tc9_bm2.out"
