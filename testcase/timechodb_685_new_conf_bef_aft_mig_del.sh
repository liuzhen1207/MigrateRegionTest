...
${cli_dir}/sbin/start-cli.sh -h ${query_ip} -e "delete from root.test.g_0.*.s_40;">${cur_dir}/del_s_40_aft_mig.out
sleep 2
 ${cli_dir}/sbin/start-cli.sh -h ${query_ip} -timeout 36000 -e "select count(s_12),count(s_23),count(s_8),count(s_40),count(s_36),count(s_9),max_time(s_17),max_time(s_29),max_time(s_8),max_time(s_49),max_time(s_36),max_time(s_9) from root.** align by device;">${cur_dir}/q_act_aft_mig.out
 v_check_res=`cat ${cur_dir}/q_act_aft_mig.out|grep root|awk -F "|" '{gsub(" ","");print $5","$6","$7}'|grep "100000,0,100000" |wc -l`
 if [[ ${v_check_res} != 40000 ]];then
    let fail_flag++
 fi
 v_check_res=`cat ${cur_dir}/q_act_aft_mig.out|grep root|awk -F "|" '{gsub(" ","");print $5","$6","$7}'|grep "100000,100000,100000" |wc -l`
 if [[ ${v_check_res} != 40000 ]];then
    let fail_flag++
 fi
