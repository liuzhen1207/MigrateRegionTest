...
function set_conf()
{
  ...
  while read line <&3
  do
     ssh ${u_name}@${line} "sudo rm -rf ${db_dir}/conf/iotdb-system.properties"
     ssh ${u_name}@${line} "sudo cp -rp ${db_dir}/conf/iotdb-system.properties.template ${db_dir}/conf/iotdb-system.properties"
     ssh ${u_name}@${line} "sudo chown -R ${u_name}:${u_name} ${db_dir}/conf/"
     ...
     ssh ${u_name}@${line} "sed -i 's/# default_schema_region_group_num_per_database=.*/default_schema_region_group_num_per_database=1/g' ${db_dir}/conf/iotdb-system.properties"
     ssh ${u_name}@${line} "sed -i 's/# default_data_region_group_num_per_database=.*/default_data_region_group_num_per_database=5/g' ${db_dir}/conf/iotdb-system.properties"
  ...
  do
    ...
    v_exist=`grep ${line} ${nodeinfo_dir}/confignode.txt|wc -l`
     if [[ ${v_exist} -gt 0 ]];then
     ...
     ssh ${u_name}@${line} "sed -i 's/# default_schema_region_group_num_per_database=.*/default_schema_region_group_num_per_database=1/g' ${db_dir}/conf/iotdb-system.properties"
     ssh ${u_name}@${line} "sed -i 's/# default_data_region_group_num_per_database=.*/default_data_region_group_num_per_database=5/g' ${db_dir}/conf/iotdb-system.properties"
     ssh ${u_name}@${line} "sed -i 's/.*atanode_memory_proportion=.*/datanode_memory_proportion=1:5:1:1:1:1/g' ${db_dir}/conf/iotdb-system.properties"
  done
